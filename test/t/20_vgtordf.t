...
is $(vg construct -r tiny/tiny.fa -v tiny/tiny.vcf.gz | vg view -tC -r 'http://example.org/' - | roqet -e "SELECT (COUNT (DISTINCT ?s) AS ?c) WHERE  {?s ?p ?o}" -F turtle -D /dev/stdin -q -r tsv | grep -c 27) 1 "There are 27 distinct subjects in the tiny.ttl tested with SPARQL"

...
is "$(samtools view small/x.bam | cut -f 1)" "$(vg inject -p x -x x.xg small/x.bam | vg surject -p x -x x.xg -t 1 - | samtools view - | sort -k 4n | cut -f 1)" \
    "vg inject retains read names"

is "$(samtools view small/x.bam | cut -f 4)" "$(vg inject -p x -x x.xg small/x.bam | vg surject -p x -x x.xg -t 1 - | samtools view - | cut -f 4 | sort)" \
    "vg inject works perfectly for the position of alignment"

is "$(samtools view small/x.bam | cut -f 5)" "$(vg inject -p x -x x.xg small/x.bam | vg surject -p x -x x.xg -t 1 - | samtools view - | sort -k 4n | cut -f 5)" \
    "vg inject works perfectly for the mappign quality of alignment"

is "$(samtools view small/x.bam | cut -f 6)" "$(vg inject -p x -x x.xg small/x.bam | vg surject -p x -x x.xg -t 1 - | samtools view - | sort -k 4n | cut -f 6)" \
    "vg inject works perfectly for the cigar of alignment"

is "$(samtools view small/x.bam | cut -f 6)" "$(vg inject -x x.xg small/x.bam | vg surject -p x -x x.xg -t 1 - | vg view -a - | sort -k 4n | cut -f 6)" \

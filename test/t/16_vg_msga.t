...
is $(vg msga -f msgas/inv.fa -w 20 -O 19 | vg mod -U 10 - | vg view -j - | jq -c '.path[] | select(.name == "inv") | .mapping[] | select(.position.is_reverse)'  | wc -l) 1 "a reference sequence set representing an inversion in it may be msga'd and detected" 

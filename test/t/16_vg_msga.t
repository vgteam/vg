...
is $(vg msga -f GRCh38_alts/FASTA/HLA/V-352962.fa -t 4 | vg mod -U 10 - | vg mod -c - | vg view - | grep ^S | cut -f 3 | sort | md5sum | cut -f 1 -d\ ) $(vg msga -f GRCh38_alts/FASTA/HLA/V-352962.fa -t 1 | vg mod -U 10 - | vg mod -c - | vg view - | grep ^S | cut -f 3 | sort | md5sum | cut -f 1 -d\ ) "graph for GRCh38 HLA-V is unaffected by the number of alignment threads"
...
is $(vg msga -f msgas/s.fa -b s1 -w 20 -N | vg view -j - | jq -M -c --sort-keys '{"node": .node, "edge": .edge}') $(vg msga -f msgas/s.fa -f msgas/s-rev.fa -b s1 -w 20 -N | vg view -j - | jq -M -c --sort-keys '{"node": .node, "edge": .edge}') "adding in existing sequences in reverse doesn't change graph"
...
vg msga -f GRCh38_alts/FASTA/HLA/K-3138.fa -w 256 -K 11 -X 1 -E 4 -Q 22 | vg validate -
is $? 0 "HLA K-3138 correctly includes all input paths"

vg msga -f msgas/cycle.fa -b s1 -w 16 -t 1 | vg validate -
is $? 0 "a difficult cyclic path can be included to produce a valid graph"

is $(vg msga -f msgas/cycle.fa -b s1 -w 16 -t 1 | vg mod -D - | vg mod -U 10 - | vg view - | grep ^S | wc -l) 2 "a cyclic path can be normalized"

vg msga -f msgas/l.fa -b a1 -w 8 -m 8 | vg validate -
is $? 0 "edges in cycles with two nodes are correctly included"

vg msga -f GRCh38_alts/FASTA/HLA/B-3106.fa -w 64 -K 11 -X 1 | vg validate -

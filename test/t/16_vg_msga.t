...
is $(vg msga -a 2 -i 2 -o 3 -e 1 -f GRCh38_alts/FASTA/HLA/V-352962.fa -t 1 -Y 64 | vg mod -n - | vg mod -n - | vg view - | grep ^S | cut -f 3 | sort | md5sum | cut -f 1 -d\ ) 16e56f0090b310d2b1479d49cf790324 "MSGA produces the expected graph for GRCh38 HLA-V"

is $(vg msga -a 2 -i 2 -o 3 -e 1 -f GRCh38_alts/FASTA/HLA/V-352962.fa -t 4 -Y 64 | vg mod -n - | vg mod -n - | vg view - | grep ^S | cut -f 3 | sort | md5sum | cut -f 1 -d\ ) 16e56f0090b310d2b1479d49cf790324 "graph for GRCh38 HLA-V is unaffected by the number of alignment threads"
...
is $(vg msga -f msgas/cycle.fa -b s1 -B 20 -t 1 -P 0.9 | vg mod -n - | vg mod -n - | vg view - | grep ^S | cut -f 3 | sort | md5sum | cut -f 1 -d\ ) \
   $(vg msga -f msgas/cycle.fa -b s1 -B 30 -t 1 -P 0.9 | vg mod -n - | vg mod -n - | vg view - | grep ^S | cut -f 3 | sort | md5sum | cut -f 1 -d\ ) \

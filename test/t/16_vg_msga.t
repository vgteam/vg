...
is $(vg msga -f GRCh38_alts/FASTA/HLA/V-352962.fa -t 1 -Y 32 | vg mod -n - | vg mod -n - | vg view - | grep ^S | cut -f 3 | sort | md5sum | cut -f 1 -d\ ) 99870bf480f9965169ed8e83989aa6c7 "MSGA produces the expected graph for GRCh38 HLA-V"

is $(vg msga -f GRCh38_alts/FASTA/HLA/V-352962.fa -t 4 -Y 32 | vg mod -n - | vg mod -n - | vg view - | grep ^S | cut -f 3 | sort | md5sum | cut -f 1 -d\ ) 99870bf480f9965169ed8e83989aa6c7 "graph for GRCh38 HLA-V is unaffected by the number of alignment threads"

...
is $(head -c 16 xy.mapping | hexdump -v -e '2/8 "%u_"') 99_99_ "empty mapping starts from the right node id"
vg prune -u -g x.gbwt -e 1 -a -m xy.mapping x.vg > /dev/null
is $(head -c 16 xy.mapping | hexdump -v -e '2/8 "%u_"') 99_127_ "the first unfolded graph adds the correct number of nodes to the mapping"
vg prune -u -g y.gbwt -e 1 -a -m xy.mapping y.vg > /dev/null
is $(head -c 16 xy.mapping | hexdump -v -e '2/8 "%u_"') 99_154_ "the second unfolded graph adds the correct number of nodes to the mapping"

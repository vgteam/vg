...
namespace vg {
	...
	bool gfa_to_graph(istream& in, VG* graph, bool only_perfect_match) {
		...
		std::function<void(gfak::sequence_elem)> seqfunc = [&](gfak::sequence_elem S){
			...
			stPinchThreadSet_addThread(pinch.get(), pinch_name, 0, S.sequence.size());
		};
		...
		std::function<void(gfak::edge_elem)> edge_func = [&](gfak::edge_elem E){
			// For each set of links, by source node name
			auto& name = E.source_name;
			...
			auto& source_string = name_to_seq[name];
			// And sequence length
			...
			for (auto& elem : cigar) {
				...
				switch(elem.second[0]) {
					...
					default:
						...
						cerr << "error:[gfa_to_graph]: GFA CIGAR invalid: " << elem.second << " operation in " << E.to_string_2() << endl;
						return false;
				}
			}

			// Work out what thread the link is to
			auto sink_pinch_name = gfa_to_pinch.translate(E.sink_name);
...
#ifdef debug
			cerr << "Found edge " << E.source_name << " = " << source_pinch_name << (source_backward ? 'L' : 'R')
				<< " -> " << E.sink_name << " = " << sink_pinch_name << (sink_backward ? 'R' : 'L') << endl;
			...
			auto& sink_string = name_to_seq[E.sink_name];
			// And sequence length
			...
			if (source_alignment_length > source_sequence_length) {
				...
				cerr << "error:[gfa_to_graph]: GFA file contains a link " << E.source_name << " " << (source_backward ? 'L' : 'R')
					<< " to " << E.sink_name  << " " << (sink_backward ? 'R' : 'L')
		...
		gg.for_each_edge_line_in_file(in, edge_func);
		...
		std::function<void(gfak::path_elem)> path_func = [&](gfak::path_elem P){
		...
		gg.for_each_path_line_in_file(in, path_func);

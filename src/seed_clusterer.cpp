        ...
        for (size_t i = 0; i < seeds.size(); i++) {
            ...
            while (snarl != nullptr) { 
                ...
                if (snarl_manager.in_nontrivial_chain(snarl)) {
                    ...
                    prev = make_pair(get_start_of(*chain).node_id(), get_start_of(*chain).backward());
        ...
        for (pair<const Snarl*, int64_t> snarl_node : snarls_in_chain) {
            ...
            if (curr_snarl != nullptr){
             ...
             //Start and end node of snarl relative to chain
                start_node = rev_in_chain ? curr_snarl->end().node_id() :
                ...
                end_length = snarl_index.nodeLength(end_node);
                ...
                if ( boundary_node_id == -1) {
                    ...
                    for (size_t i = 0; i < snarl_clusters.size(); i++) {
                        ...
                        for (size_t j = 0 ; j < new_cluster_indices.size(); j++) {
                            ...
                            for (size_t k : new_cluster.first) { 
                                ...
                                if (!in_this_cluster) {
                                    ...
                                    if (abs(cluster.dist_right -
                                          (end_length - boundary_cluster.dist_left))
                                          < distance_limit) {
                                        in_this_cluster = true;
                                        curr_cluster_assignments.insert(j);
                                    } 
                    ...
                    for (pair<hash_set<size_t>, hash_set<size_t>> 
                        ...
                        for (size_t i : cluster_indices.first) {
                            ...
                            int64_t old_dist_left = boundary_cluster.dist_left ==-1
                                         || boundary_offset == -1 ? -1 : 
                            ...
                            curr_cluster.dist_right = DistanceIndex::minPos({
                             boundary_cluster.dist_right, curr_cluster.dist_right});
                        }
                        for (size_t i : cluster_indices.second) {
                            ...
                            curr_cluster.dist_left = DistanceIndex::minPos({
                                    old_cluster.dist_left, curr_cluster.dist_left});
                            curr_cluster.dist_right = DistanceIndex::minPos({
                               old_cluster.dist_right, curr_cluster.dist_right});
                        }
                ...
                //Find if this node is traversed forward or backward in the chain
                bool rev_in_snarl = boundary_node_id == snarl_index.snarlStart.first
                     ? snarl_index.snarlStart.second : snarl_index.snarlEnd.second;
                bool rev_in_chain = snarl_manager.chain_orientation_of(sn) ? 
                                            !rev_in_snarl : rev_in_snarl;
                curr_clusters = 
                     get_clusters_node( seeds, seed_list, node_to_seed,
                             distance_limit, snarl_manager, dist_index, 
                             boundary_node_id, rev_in_chain, node_length) ;
                ...
                end_length = node_length;
                start_node = boundary_node_id;
                end_node = boundary_node_id;
            }
#ifdef DEBUG

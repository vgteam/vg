...
tuple<vector<SnarlTraversal>, int, const string, size_t, size_t> FlowCaller::get_traversals(
    const Snarl& managed_snarl, int ploidy) {
...
bool FlowCaller::call_snarl(const Snarl& managed_snarl, int ploidy) {
    ...
    tuple<vector<SnarlTraversal>, int, const string&, size_t, size_t> found_travs = get_traversals(managed_snarl, ploidy);
...
tuple<vector<SnarlTraversal>, int, const string, size_t, size_t> NestedFlowCaller::get_traversals(
    const HandleGraph& trav_graph, const Snarl& managed_snarl, int ploidy) {
    ...
    // note: we are potentially using a different graph here:
    unique_ptr<TraversalFinder> traversal_finder = get_trav_finder(trav_graph);
...
bool NestedFlowCaller::call_snarl_recursive(const Snarl& managed_snarl, int ploidy) {
    ...
    // recurse on the children, filling up snarl_table
    const vector<const Snarl*>& children = snarl_manager.children_of(&managed_snarl);
    
    size_t child_count = 0;
    for (const Snarl* child : children) {
        if (!snarl_manager.is_trivial(&managed_snarl, graph)) {
            ++child_count;
            call_snarl_recursive(*child, ploidy);
        }
    }
    
    // make a snarl graph.  this is a simplification of the netgraph that ignores chains.
    // all edges inside a snarl are invisible.  virtual edges are added from the snarl's
    // start handle to all the handles that are attached to its end handle, and vice versa
    SnarlGraph snarl_graph(&graph, snarl_manager, children);

    // remember the graph so that the support callbacks can get access to it
    snarl_graphs[omp_get_thread_num()] = child_count > 0 ? &snarl_graph : nullptr;

    // compute the traversals along with a bunch of reference path information
    tuple<vector<SnarlTraversal>, int, const string&, size_t, size_t> found_travs = get_traversals(
        child_count > 0 ? (const HandleGraph&)snarl_graph : (const HandleGraph&)graph, managed_snarl, ploidy);
    ...
    if (traversals_only) {
        ...
        // update the table
        vector<NestInfo>& nest_info_by_ploidy = snarl_table[make_pair(managed_snarl.start(), managed_snarl.end())];
        if (nest_info_by_ploidy.size() < ploidy) {
            nest_info_by_ploidy.resize(ploidy);
        }

        // use our support caller to choose our genotype
        for (int comp_ploidy = 1; comp_ploidy <= ploidy; ++comp_ploidy) {
            NestInfo& nest_info = nest_info_by_ploidy[comp_ploidy - 1];
            vector<int> trav_genotype;
            unique_ptr<SnarlCaller::CallInfo> trav_call_info;
            std::tie(trav_genotype, trav_call_info) = snarl_caller.genotype(managed_snarl, travs, ref_trav_idx, comp_ploidy, ref_path_name,
                                                                            make_pair(get<3>(found_travs), get<4>(found_travs)));

            assert(trav_genotype.empty() || trav_genotype.size() == comp_ploidy);

            // use our support caller to get some basic support stats for the genotype
            int max_trav_size = dynamic_cast<PoissonSupportSnarlCaller::PoissonCallInfo*>(trav_call_info.get())->max_trav_size;

            // get the genotype support
            SupportBasedSnarlCaller& support_caller = dynamic_cast<SupportBasedSnarlCaller&>(snarl_caller);
            vector<Support> genotype_supports = support_caller.get_support_finder().get_traversal_genotype_support(travs, trav_genotype, {}, 0, &max_trav_size);
        
            // Get the depth of the site
            Support total_site_support = std::accumulate(genotype_supports.begin(), genotype_supports.end(), Support());    
            double total_site_depth = SupportBasedSnarlCaller::support_val(total_site_support);

            // Update the table so parent snarls can use the calls recursively.
            nest_info.support = total_site_depth;
            nest_info.traversals = travs;
            nest_info.genotype = trav_genotype;
            nest_info.call_info.reset(trav_call_info.release());
        }
    }
...
bool NestedFlowCaller::call_snarl(const Snarl& managed_snarl, int ploidy) {
    ...
    // compute the traversals along with a bunch of reference path information
    tuple<vector<SnarlTraversal>, int, const string&, size_t, size_t> found_travs = get_traversals(graph, managed_snarl, ploidy);
...
void NestedFlowCaller::init_table() {

    // update the snarls
    snarl_manager.for_each_snarl_preorder([&](const Snarl* snarl) {
            snarl_table[make_pair(snarl->start(), snarl->end())] = vector<NestInfo>();
        });

    // update the graph pointers that we use to get support
    snarl_graphs.resize(get_thread_count(), nullptr);
}

double NestedFlowCaller::get_nested_node_support(handle_t handle) {
    const SnarlGraph* snarl_graph = snarl_graphs[omp_get_thread_num()];
    if (snarl_graph != nullptr) {
        pair<bool, handle_t> snarl_node = snarl_graph->node_to_snarl(handle);
        if (snarl_node.first == true) {
            // we pull it from the table
            Visit v1;
            v1.set_node_id(graph.get_id(handle));
            v1.set_backward(graph.get_is_reverse(handle));
            Visit v2;
            v2.set_node_id(graph.get_id(snarl_node.second));
            v2.set_backward(graph.get_is_reverse(snarl_node.second));
            auto i = snarl_table.find(make_pair(v1, v2));
            if (i == snarl_table.end()) {
                swap(v1, v2);
                v1.set_backward(!v1.backward());
                v2.set_backward(!v2.backward());
                i = snarl_table.find(make_pair(v1, v2));
                assert(i != snarl_table.end());
            }
            // todo: parameterize
            return i->second.back().support;
        }
    }
    // calling method should fall-back on another structure, as the
    // handle does not correspond to a known chain
    return -1;
}

edge_t NestedFlowCaller::resolve_chain_edge(edge_t edge) {
    const SnarlGraph* snarl_graph = snarl_graphs[omp_get_thread_num()];
    if (snarl_graph != nullptr) {
        tuple<bool, handle_t, edge_t> snarl_edge = snarl_graph->edge_to_snarl_edge(edge);
        if (get<0>(snarl_edge) == true) {
            return get<2>(snarl_edge);
        }
    }
    return edge;
}
    ...
    for (const Snarl* snarl : snarls) {
        if (!snarl_manager.is_trivial(snarl, *backing_graph)) {
            this->snarls[backing_graph->get_handle(snarl->start().node_id(), snarl->start().backward())] =
                backing_graph->get_handle(snarl->end().node_id(), snarl->end().backward());
            this->snarls[backing_graph->get_handle(snarl->end().node_id(), !snarl->end().backward())] =
                backing_graph->get_handle(snarl->start().node_id(), !snarl->start().backward());
        }
    }
}
...
tuple<bool, handle_t, edge_t> SnarlGraph::edge_to_snarl_edge(edge_t edge) const {
    ...
    if (i != snarls.end()) {
        ...
        if (i != snarls.end()) {
            ...
            out_edge.second = backing_graph->flip(i->second);
...
bool SnarlGraph::follow_edges_impl(const handle_t& handle, bool go_left, const std::function<bool(const handle_t&)>& iteratee) const {
    if (!go_left) {
        ...
        if (i == snarls.end()) {
            ...
            return backing_graph->follow_edges(i->second, go_left, iteratee);
        }
    } else {
...
NestedTraversalSupportFinder::NestedTraversalSupportFinder(const HandleGraph& graph, SnarlManager& snarl_manager,
                                                           TraversalSupportFinder& support_finder, NestedFlowCaller** flow_caller) :
    TraversalSupportFinder(graph, snarl_manager),
    graph(graph),
    backing_support_finder(support_finder),
    flow_caller(flow_caller) {
}

NestedTraversalSupportFinder::~NestedTraversalSupportFinder() {
}

Support NestedTraversalSupportFinder::get_edge_support(const edge_t& edge) const {
    return backing_support_finder.get_edge_support(edge);
}

Support NestedTraversalSupportFinder::get_edge_support(id_t from, bool from_reverse,
                                                       id_t to, bool to_reverse) const {
    return backing_support_finder.get_edge_support(from, from_reverse, to, to_reverse);
}

Support NestedTraversalSupportFinder::get_min_node_support(id_t node) const {
    return backing_support_finder.get_min_node_support(node);
}

Support NestedTraversalSupportFinder::get_avg_node_support(id_t node) const {
    return backing_support_finder.get_avg_node_support(node);
}

size_t NestedTraversalSupportFinder::get_avg_node_mapq(id_t node) const {
    return backing_support_finder.get_avg_node_mapq(node);
}

tuple<Support, Support, int> NestedTraversalSupportFinder::get_child_support(const Snarl& snarl) const {
    Support support;
    // tood: set!
    int length = 1;
    double support_v = (*flow_caller)->get_nested_node_support(graph.get_handle(snarl.start().node_id(), snarl.start().backward()));
    support.set_forward(support_v);
    return make_tuple(support, support, length);
}

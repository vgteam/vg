...
pos_t nth_position(const Path& path, int64_t n, bool from_end, bool ignore_softclip) {
    ...
    if (path.mapping_size()) {
        if (from_end) {
            ...
            if (ignore_softclip && final_mapping.edit(final_mapping.edit_size() - 1).from_length() == 0) {
                // we have to walk further to skip the softclips
                n += final_mapping.edit(final_mapping.edit_size() - 1).to_length();
            }
            int64_t i = path.mapping_size() - 1;
            while (i >= 0 && n < mapping_to_length(path.mapping(i))) {
                n -= mapping_to_length(path.mapping(i));
                --i;
            }
            if (i < 0) {
                pos = initial_position(path);
            }
            else {
                const Mapping& mapping = path.mapping(i);
                int64_t j = mapping.edit_size() - 1;
                int64_t from_length = 0;
                while (j >= 0 && n < mapping.edit(j).to_length()) {
                    n -= mapping.edit(j).to_length();
                    from_length += mapping.edit(j).from_length();
                    --j;
                }
                if (j >= 0) {
                    from_length += (n * mapping.edit(j).from_length()) / mapping.edit(j).to_length();
                }
                const Position& position = mapping.position();
                get_id(pos) = position.node_id();
                get_is_rev(pos) = position.is_reverse();
                get_offset(pos) = position.offset() + mapping_from_length(mapping) - from_length;
            }
        }
        else {
            if (ignore_softclip && path.mapping(0).edit(0).from_length() == 0) {
                // we have to walk further to skip the softclips
                n += path.mapping(0).edit(0).to_length();
            }
            int64_t i = 0;
            while (i < path.mapping_size() && n < mapping_to_length(path.mapping(i))) {
                n -= mapping_to_length(path.mapping(i));
                ++i;
            }
            if (i == path.mapping_size()) {
                ...
                int64_t j = 0;
                int64_t from_length = 0;
                while (j < mapping.edit_size() && n < mapping.edit(j).to_length()) {
                    n -= mapping.edit(j).to_length();
                    from_length += mapping.edit(j).from_length();
                    ++j;
                }
                ...
                get_id(pos) = position.node_id();
                get_is_rev(pos) = position.is_reverse();
                get_offset(pos) = position.offset() + from_length;
            }
        }
    }
    return pos;

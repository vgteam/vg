...
// If we know the graph is a single weakly connected component, single_component
// can be set to ture to avoid recomputing components. Notes:
//  - returned cactus graph needs to be freed by stCactusGraph_destruct
//  - returns a Cactus graph, and a list of stCactusEdgeEnd* telomeres, in pairs
//  of adjacent items.
pair<stCactusGraph *, stList *>
handle_graph_to_cactus(const PathHandleGraph &graph,

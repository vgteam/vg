...
string supplementary_annotation(const Alignment& supplementary) {
    ...
    // encode CIGAR
    for (auto& cigar_op : graph_cigar(supplementary)) {
        strm << cigar_op.first << cigar_op.second;
    }
...
vector<Alignment> decode_supplementary_annotation(const Alignment& primary, const HandleGraph& graph) {
    ...
    if (has_annotation(primary, "supplementaries")) {

        const auto supp_annotations = get_annotation<string>(primary, "supplementaries");

        supplementaries.reserve(count(supp_annotations.begin(), supp_annotations.end(), ';'));

        size_t i = 0;
        while (i < supp_annotations.size()) {
            size_t j = supp_annotations.find(';', i);

            auto annotation = supp_annotations.substr(i, j - i);

            supplementaries.emplace_back();
            auto& supp = supplementaries.back();
            ...
            set_annotation(supp, "supplementary", true);
            ...
            while (annotation[k] != ',') {
                ...
                pos->set_node_id(parse<int64_t>(annotation.substr(k, l - k)));
                pos->set_offset(0);
                k = l;
            }
            ...
            first_pos->set_offset(parse<int64_t>(annotation.substr(k, l - k)));
            ...
            while (annotation[k] != ',') {
                ...
                int64_t op_len = parse<int64_t>(annotation.substr(k, l - k));
                ...
                switch (op) {
                    ...
                    case '=':
                    {
                        while (op_len != 0) {
                            ...
                            size_t node_len = graph.get_length(graph.get_handle(path->mapping(mapping_idx).position().node_id()));
            ...
            supp.set_mapping_quality(parse<int32_t>(annotation.substr(k, l - k)));
            // parse the score
            k = l + 1;
            supp.set_score(parse<int32_t>(annotation.substr(k, annotation.size() - k)));

...
string Deconstructor::get_vcf_header() {
    ...
    // find sample names from non-reference paths (both generic and haplotype)
    graph->for_each_path_of_sense({PathSense::GENERIC, PathSense::HAPLOTYPE}, [&](const path_handle_t& path_handle) {
        string path_name = graph->get_path_name(path_handle);
        if (!this->ref_paths.count(path_name) && !Paths::is_alt(path_name)) {

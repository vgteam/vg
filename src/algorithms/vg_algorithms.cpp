...
namespace algorithms {
    unordered_map<id_t, id_t> extract_connecting_graph_internal(Graph& g, int64_t max_len,
...
#ifdef debug_vg_algorithms
                cerr << "colocation: shared node reachable" << endl;
#endif
            }
            else if (is_rev(pos_1) == is_rev(pos_2)) {
                colocation = SharedNodeUnreachable;
#ifdef debug_vg_algorithms
                cerr << "colocation: shared node unreachable" << endl;
#endif
            }
            else {
                colocation = SharedNodeReverse;
#ifdef debug_vg_algorithms
                cerr << "colocation: shared node reverse" << endl;
#endif
            }
        }
        else {
...
#ifdef debug_vg_algorithms
            cerr << "colocation: separate nodes" << endl;
#endif
        }
        ...
        int64_t last_rev_traversal_length = offset(pos_2) - graph[id(pos_2)].sequence.size();
...
#ifdef debug_vg_algorithms
        cerr << "forward max len: " << forward_max_len << ", backward max len " << backward_max_len << endl;
        cerr << "first trav len " << first_traversal_length << " last trav len " << last_traversal_length << " last rev trav len " << last_rev_traversal_length << endl;
#endif
...
#ifdef debug_vg_algorithms
                    cerr << "FORWARD SEARCH: edge goes to " << next_id << " in " << (next_rev ? "reverse" : "forward") << " orientation" << endl;
#endif
        ...
        if (detect_terminal_cycles && found_target &&
            ...
            while (!queue.empty()) {
                ...
                for (Edge& edge : (trav.rev ? edges_on_start(trav.id) : edges_on_end(trav.id))) {
...
#ifdef debug_vg_algorithms
                    cerr << "BACKWARD SEARCH: edge goes to " << next_id << " in " << (next_rev ? "reverse" : "forward") << " orientation" << endl;
#endif
                    ...
                    // the only edges we will not have seen are to new nodes or self-loops on the final node
                    // (because we pre-excluded cases where we would have seen self loops in the forward traversal)
                    if (!graph.count(next_id) ||
                        (trav.id == id(pos_2) && next_id == id(pos_2))) {
                        // what side does this edge enter on the next node?
                        auto& edges_in = next_rev ? graph[next_id].edges_right : graph[next_id].edges_left;
                        // is the edge reversing?
                        bool reversing = (trav.rev != next_rev);
        ...
        auto trimmed_seq_left = [](const string& seq, int64_t offset, bool rev) {
            if (rev) {
                return seq.substr(seq.size() - offset, offset);
            }
            else {
                return seq.substr(0, offset);
            }
        };
                    ...
                    if (in_cycle_1) {
                        ...
                        for (pair<id_t, bool>& edge : node_1.edges_right) {
                            if (edge.first == id(pos_1) && edge.second) {
                                ...
                                // copy the edge from the righthand node and add backward reference
                                new_node.edges_right.emplace_back(edge.first, edge.second);
                                node_1.edges_right.emplace_back(next_id, edge.second);
                    ...
                    if (in_cycle_2) {
                        ...
                        for (pair<id_t, bool>& edge : node_2.edges_right) {
                            if (edge.first == id(pos_2) && edge.second) {
                                ...
                                // copy the edge from the righthand node and add backward reference
                                new_node.edges_right.emplace_back(edge.first, edge.second);
                                node_2.edges_right.emplace_back(next_id, edge.second);
                                // make another edge for the nonreversing self loop on only the cyclic node
                                new_node.edges_right.emplace_back(next_id, edge.second);
                                new_node.edges_left.emplace_back(next_id, edge.second);
                            }
                    ...
                    if (in_cycle_1) {
                        ...
                        for (pair<id_t, bool>& edge : lefthand_new_edges) {
                            if (edge.first == id(pos_1)) {
                                // if this is a self loop, update it to the new node
                        ...
                        for (pair<id_t, bool>& edge : righthand_new_edges) {
                            if (edge.first == righthand_id) {
                                ...
                                lefthand_new_edges.emplace_back(next_id, edge.second);
                                // make another edge for the nonreversing self loop on only the cyclic node
                                edges_out.emplace_back(next_id, edge.second);
                ...
                case SharedNodeUnreachable:
                {
                    ...
                    for (pair<id_t, bool>& edge : old_outgoing_edges) {
                        if (edge.first == id(pos_1) && edge.second) {
                            ...
                            old_incoming_edges.emplace_back(next_id, edge.second);
                            // make another edge for the nonreversing self loop on only the cyclic node
                            new_outgoing_edges.emplace_back(next_id, edge.second);
        ...
        switch (colocation) {
            ...
            case SharedNodeReachable:
            {
                ...
                // cut the node sequence
                if (is_rev(pos_1)) {
                    node.sequence = node.sequence.substr(node.sequence.size() - offset(pos_2),  offset(pos_2) - offset(pos_1) - 1);
                }
                else {
                    node.sequence = node.sequence.substr(offset(pos_1) + 1, offset(pos_2) - offset(pos_1) - 1);
                }
                break;
            }
            case SharedNodeUnreachable:
...
#ifdef debug_vg_algorithms
        cerr << "state of graph before tip removal:" << endl;
        for (const pair<id_t, LocalNode>& node_record : graph) {
            cerr << node_record.first << ": L( ";
        ...
        // STEP 5: REMOVING TIPS
        // now all cycles to the original (non-duplicated) nodes are dangling tips. there may also be
        // tips that resulted from paths we explored until hitting the max search length in the graph
        // extraction step. next we remove all tips (except if the tip is a node with our end position on it)
        ...
        for (const pair<id_t, int64_t>& left_deg_record : left_degree) {
            ...
            while (!to_check.empty()) {
                id_t next_id = to_check.back();
                to_check.pop_back();
                if (next_id == id(pos_1) || next_id == id(pos_2) ||
                    next_id == duplicate_node_1 || next_id == duplicate_node_2 ||
                    !graph.count(next_id)) {
                    // the end nodes get a free pass on begin tips, or we may
                    // have already pruned this node
                    continue;
                }
                if (left_degree[next_id] == 0) {
#ifdef debug_vg_algorithms
                    cerr << "TIP REMOVAL: node " << next_id << " is a left tip" << endl;
#endif
                    for (pair<id_t, bool>& edge : graph[next_id].edges_right) {
                        if (edge.second) {
                ...
                else if (right_degree[next_id] == 0) {
#ifdef debug_vg_algorithms
                    cerr << "TIP REMOVAL: node " << next_id << " is a right tip" << endl;
#endif
                    for (pair<id_t, bool>& edge : graph[next_id].edges_left) {
                        if (edge.second) {
                            left_degree[edge.first]--;
                        }
                        else {
...
#ifdef debug_vg_algorithms
        cerr << "state of graph after tip removal:" << endl;
        for (const pair<id_t, LocalNode>& node_record : graph) {
            cerr << node_record.first << ": L( ";
            for (pair<id_t, bool> edge : node_record.second.edges_left) {
                cerr << edge.first << (edge.second ? "+" : "-") << " ";
        ...
        // STEP 6: PRUNING LONG PATHS
        // now all remaining nodes and edges are on some path from pos_1 to pos_2. it is still possible that
        // some of them are only on paths that are above our maximum distance, so we do a forward-backward
        // distance search to check
        
        // edges stored as ((from, from_start), (to, to_end))
        unordered_set<pair<pair<id_t, bool>, pair<id_t, bool>>> reachable_edges;
        
        // put an edge from a given side of a node into canonical form for the hash
        ...
        if (duplicate_node_1) {
            queue.emplace(duplicate_node_1, is_rev(pos_1), first_traversal_length);
        ...
        queue.emplace(id(pos_2), !is_rev(pos_2), last_rev_traversal_length);
        ...
        if (duplicate_node_2) {
            queue.emplace(duplicate_node_2, !is_rev(pos_2), last_rev_traversal_length);
            queued_traversals.insert(make_pair(duplicate_node_1, !is_rev(pos_1)));
        }
        ...
        for (auto n : graph) {
            ...
            if (forward_trav_dist.count(make_pair(n.first, false))) {
                cerr << " f-:" << forward_trav_dist[make_pair(n.first, false)];
            }
            else {
                cerr << " f-:.";
            }
            if (reverse_trav_dist.count(make_pair(n.first, true))) {
                cerr << " r+:" << reverse_trav_dist[make_pair(n.first, true)];
            }
            else {
                cerr << " r+:.";
            }
            if (reverse_trav_dist.count(make_pair(n.first, false))) {
                cerr << " r-:" << reverse_trav_dist[make_pair(n.first, false)];
            }
            else {
                cerr << " r-:.";
            }
        ...
        // STEP 7: TRANSLATION TO PROTOBUF
    ...
    unordered_map<id_t, id_t> extract_connecting_graph(VG& vg, Graph& g, int64_t max_len,
        ...
        return extract_connecting_graph_internal(g, max_len, pos_1, pos_2, detect_terminal_cycles,
                                                 [&](id_t id) {

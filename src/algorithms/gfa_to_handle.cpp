...
void validate_gfa_edge(const gfak::edge_elem& e) {
    ...
    if (e.source_begin != e.source_end || e.sink_begin != 0 || e.sink_end != 0) {
        throw BluntGFAFormatError(not_blunt + " Found edge with an overlay: " + e.source_name + "[" + to_string(e.source_begin) + ":" + to_string(e.source_end) + "] -> " + e.sink_name + "[" + to_string(e.sink_begin) + ":" + to_string(e.sink_end) + "]");
    }
...
bool gfa_to_vg(istream& in, VG* graph, bool only_perfect_match) {
...
void gfa_to_handle_graph_load_graph(const string& filename, istream* unseekable, MutableHandleGraph* graph,
    ...
    if (!unseekable) {
        ...
        gfa_to_vg(in_file, &vg_graph, false);
        copy_path_handle_graph(&vg_graph, dynamic_cast<MutablePathMutableHandleGraph*>(graph));
    } else {

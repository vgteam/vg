...
void three_edge_connected_component_merges_dense(
    size_t node_count, size_t first_root,
    const function<void(size_t, const function<void(size_t)> &)>
  ...
  auto absorb_all_along_path = [&](number_t into, number_t path_start,
                                   number_t path_past_end) {

#ifdef debug
    cerr << "(Absorbing all along path into " << into << " from " << path_start
         << " to before " << path_past_end << ")" << endl;
#endif
...
#endif
        // Only allowed if the path was actually edge-free and no merges needed
        // to happen.
        assert(path_null);
  ...
  while (next_unvisited != node_count) {
    ...
    while (!stack.empty()) {
      // While there's still nodes on the DFS stack from the last component we
      // broke into Grab the stack frame. Note that this reference will be
      // invalidated if we add stuff to the stack!
...
#ifdef debug
        cerr << "\tPut " << frame.neighbors.size() << " edges on to do list"
             << endl;
#endif
        ...
        if (!frame.neighbors.empty()) {
...
#ifdef debug
          cerr << "Return to node " << frame.current << " with more edges to do"
               << endl;
          ...
          if (!frame.recursing) {
...
#ifdef debug
            cerr << "\tThink of edge to neighbor " << neighbor_number
            ...
            if (!neighbor.visited) {
              ...
              // No need to recurse.This is either a back-edge or the back side
              // of the tree edge to the parent.

              if (stack.size() > 1 &&
                  neighbor_number == stack[stack.size() - 2].current &&
                  !frame.saw_parent_tree_edge) {
                // This is the edge we took to get here (tree edge)
#ifdef debug
                cerr << "\t\tNeighbor is parent; this is the tree edge in."
                     << endl;
#endif

                // For tree edges, since they aren't either kind of back edge,
                // neither 1.2 nor 1.3 fires. But the next edge to the parent
                // will be a back edge.
...
#ifdef debug
                cerr << "\t\tNeighbor is upstream of us (outgoing back edge)."
                     << endl;
#endif
                ...
                if (neighbor.dfs_counter < node.low_point) {
                  ...
                  cerr << "\t\t\tNeighbor has a lower low point ("
                       << neighbor.dfs_counter << " < " << node.low_point << ")"
                       << endl;

                  cerr << "\t\t\t\tAbsorb along path to old low point source"
                       << endl;
#endif
...
#ifdef debug
                  cerr << "\t\t\t\tNew lower low point " << node.low_point
                       << endl;
#endif
...
#ifdef debug
                  cerr << "\t\t\t\tNew path " << path_to_string(frame.current)
                       << endl;
#endif
...
#ifdef debug
                cerr << "\t\tWe are upstream of neighbor (incoming back edge)."
                     << endl;
#endif
...
#ifdef debug
                cerr << "\t\t\tDrop degree to " << node.effective_degree
                     << endl;

                cerr << "\t\t\tWant to absorb along path towards low point "
                        "source through neighbor"
                     << endl;
#endif
                ...
                // Start out with ourselves as the replacement neighbor
                // ancestor.
                number_t replacement_neighbor_number = frame.current;
                // Consider the next candidate
                number_t candidate =
                    nodes[replacement_neighbor_number].path_tail;
                while (candidate != numeric_limits<number_t>::max() &&
                  ...
                  // This candidate is a lower ancestor of the neighbor, so
                  // adopt it.
...
#ifdef debug
                cerr << "\t\t\tNeighbor currently belongs to node "
                     << replacement_neighbor_number << endl;

                cerr << "\t\t\tAbsorb along path towards low point source "
                        "through there"
                     << endl;
#endif
...
#ifdef debug
            cerr << "\tReturned from recursion on neighbor " << neighbor_number
                 << endl;
#endif
            ...
            if (neighbor.low_point == neighbor.dfs_counter) {
              // It has no back-edges out of its own subtree, so it must be
              // across a bridge.
#ifdef debug
              cerr << "\t\tNeighbor is across a bridge edge! Hide edge!"
                   << endl;
#endif
              ...
              // Wasn't a bridge edge, so we care about more than just
              // traversing that part of the graph.

              // Do steps 1.1.1 and 1.1.2 of the algorithm as described in the
              // paper.
              if (neighbor.effective_degree == 2) {
                ...
                cerr << "\t\t\tEdge " << frame.current << "-" << neighbor_number
                     << " should never be seen again" << endl;
#endif
...
#ifdef debug
                cerr << "\t\t\tNew neighbor path: "
                     << path_to_string(neighbor_number) << endl;
#endif
              }

              // Because we hid the bridge edges, degree 1 nodes should never
              // happen
              ...
              if (node.low_point <= neighbor.low_point) {
...
#ifdef debug
                cerr << "\t\tWe have a sufficiently low low point; neighbor "
                        "comes back in in our subtree"
                     << endl;

                cerr
                    << "\t\t\tAbsorb us and then the neighbor's path to the end"
                    << endl;
#endif
                ...
                // end.
                absorb_all_along_path(frame.current, neighbor_number,
                                      numeric_limits<number_t>::max());
              } else {
#ifdef debug
                cerr << "\t\tNeighbor has a lower low point ("
                     << neighbor.low_point << " < " << node.low_point
                     << "); comes back in outside our subtree" << endl;
#endif
                ...
                cerr << "\t\t\tAbsorb along path to old low point soure"
                     << endl;
#endif
...
#ifdef debug
          cerr << "\tNode is visited and no neighbors are on the to do list."
               << endl;
...
void three_edge_connected_components_dense(
    size_t node_count, size_t first_root,
    const function<void(size_t, const function<void(size_t)> &)>
        &for_each_connected_node,
    const function<void(const function<void(const function<void(size_t)> &)> &)>
        &component_callback) {
  ...
  three_edge_connected_component_merges_dense(
      node_count, first_root, for_each_connected_node, [&](size_t a, size_t b) {
...
void three_edge_connected_components_dense_cactus(
    size_t node_count,
    const function<void(size_t, const function<void(size_t)> &)>
        &for_each_connected_node,
    const function<void(const function<void(const function<void(size_t)> &)> &)>
  ...
  // Make the stList of all the vertices, where each vertex is an stList of
  // single element stIntTuple items that point to the ranks of connected nodes.
  // When an item is removed, use the list destructor on it.
  stList *vertices = stList_construct3(0, (void (*)(void *))stList_destruct);

  // TODO: No way to hint final size to the list, and we need the individual
  // member lists to know their destructors for their elements.
  ...
  for (size_t rank = 0; rank < node_count; rank++) {
    while (rank >= stList_length(vertices)) {
      ...
      // When an item in the node's adjacency list is destroyed, run the int
      // tuple destructor.
      stList_append(vertices, stList_construct3(
    ...
    for_each_connected_node(rank, [&](size_t other_rank) {
      ...
      // For each edge on the node, represent it as a 1-tuple in the node's
      // list.
      stList_append((stList *)stList_get(vertices, rank),
                    stIntTuple_construct1((int64_t)other_rank));
      // We don't have to do the back-edge now; we will do it when we visit the
      // other node.
    });
  }
...
#ifdef debug
        cerr << "Component " << i << " contains node "
             << stIntTuple_get((stIntTuple *)stList_get(component, j), 0)
             << endl;
#endif

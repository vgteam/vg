...
vector<vector<handle_t>> find_haplotypes_not_at_source(const GBWTGraph haploGraph, unordered_set<handle_t> touched_handles, const id_t& sink_id){
    ...
    for (handle_t handle : touched_handles){
        cerr << "in not_at_source: " << haploGraph.get_sequence(handle) << endl;
...
pair<vector<string>, vector<string>> depth_first_haplotypes_to_strings(const HandleGraph& graph, const id_t& source_id, const id_t& sink_id){
    ...
    string gbwt_name = "test/robin_haplotypes/threads_in_middle_example/chr10_subgraph_0_new_2.gbwt";
...
// -----------------------------------------------------------------------------------------
    ...
    // cerr << endl << "initial test" << endl << endl;
    // handle_t test_handle = haploGraph.get_handle(23495);

    // cerr << "got the test handle?" << haploGraph.get_id(test_handle) << haploGraph.get_sequence(test_handle) << endl;
    // cerr << "\n follow edges!" << endl;
    // haploGraph.follow_edges(test_handle, true, [&](const handle_t next_handle) {
    //     cerr << "next handles: " << haploGraph.get_sequence(next_handle) << haploGraph.get_id(next_handle) << endl;
    // });
    // cerr << "end follow edges!\n " << endl;

    // cerr << "\n follow paths!" << endl;
    // haploGraph.follow_paths(haploGraph.get_state(test_handle), [&](const gbwt::SearchState next_state) -> bool {
    //     handle_t next_handle = haploGraph.node_to_handle(next_state.node);
    //     cerr << "next handles: " << haploGraph.get_sequence(next_handle) << haploGraph.get_id(next_handle) << endl;
    //     return true;
    // });
    // cerr << "end follow paths!\n " << endl;

    // cerr << "\n how many paths overlap test_state?" << endl;
    // gbwt::SearchState test_state = haploGraph.get_state(test_handle);
    // cerr << "state.size 23495" << test_state.size() << endl;
    // handle_t test_handle_2 = haploGraph.get_handle(23494);
    // gbwt::SearchState test_state_2 = haploGraph.get_state(test_handle_2);
    // cerr << "state.size 23494" << test_state.size() << endl;

    // handle_t test_handle_3 = haploGraph.get_handle(23493);
    // gbwt::SearchState test_state_3 = haploGraph.get_state(test_handle_3);
    // cerr << "state.size 23493" << test_state.size() << endl;

    // cerr << "rightmost node. (not just 'node to the right'?" << haploGraph.get_sequence(haploGraph.node_to_handle(test_state.node)) << endl;
    // cerr << "end test battery" << endl;
    ...
    while (!haplotype_queue.empty()) {
        ...
        // cerr << "\n\n follow edges!" << endl;
        // haploGraph.follow_edges(cur_haplotype.first.back(), false, [&](const handle_t next_handle) {
        //     cerr << "next handles: " << haploGraph.get_sequence(next_handle) << haploGraph.get_id(next_handle) << endl;
        // });
        // cerr << "end follow edges!\n\n " << endl;
        ...
        // or one of the finished haplotype_handle_vecs.
        if (next_searches.size() > 1){
            ...
            for (gbwt::SearchState next_search : next_searches){
                ...
                if (haploGraph.get_id(next_handle) == sink_id){
                    cerr << "anpparently next handle of node " << haploGraph.get_id(cur_haplotype.first.back()) << " is sink" << endl;
    ...
    for (auto handle : touched_handles){
        cerr << "\n\n\nin starting at source " << haploGraph.get_sequence(handle) << endl;
    }

    //TODO: make following code into a separate function - the convert_handle_vec_to_strings, or something.

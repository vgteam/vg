...
void stack_up_valid_walks(VG& graph, NodeTraversal walk_head, list<NodeTraversal>& stack) {
...
vector<SiteTraversal> ExhaustiveTraversalFinder::find_traversals(const NestedSite& site) {
    ...
    map<NodeTraversal, NestedSite*> child_site_starts;
    map<NodeTraversal, NestedSite*> child_site_ends;
    ...
    while (stack.size()) {
        ...
        NodeTraversal node_traversal = stack.pop_back();
        ...
        SiteTraversal::Visit visit;
        // have we finished a traversal through the site?
        if (node_traversal == site.end) {
            // construct final visit
            visit.node = top.node;
            visit.backward = top.backward;
        ...
        if (child_site_starts.count(node_traversal)) {
            ...
            NestedSite* child_site = child_site_starts[node_traversal];
            ...
            // make a visit out of the site
            NestedSite* child_site = child_site_ends[node_traversal];
            visit.child = child_site;
            visit.backward = true;
            ...
            // translate into a visit
            visit.node = node_traversal.node;
            visit.backward = node_traversal.backward;
...
#pragma omp critical (cerr)
                    ...
                    if (child_site_starts.count(node_traversal)) {
                        ...
                        allele_stream << "(" << child_site->start->id() << ":" << child_site->end->id() << ")";
                        
                    }
                    else if (child_site_ends.count(node_traversal)) {
                        NestedSite*  = child_site_starts[node_traversal];
                        ...
                        allele_stream << "(" << child_site->end->id() << ":" << child_site->start->id() << ")";
                    }
                    else {
    ...
    for(auto& result : results) {
        ...
        to_return.back().visits = std::move(traversals);
    }

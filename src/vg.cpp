...
void VG::from_istream(istream &in, bool showp, bool warn_on_duplicates) {
  ...
  if (in.good()) {
    ...
    // It's entirely possible that none of that worked. So clear the error flags
    // and leave the size at 0.
    in.clear();
  }
  ...
  function<void(Graph &)> lambda = [this, &in, &file_size,
                                    &warn_on_duplicates](Graph &g) {
    ...
    // We usually expect these to not overlap in nodes or edges, so complain
...
VG::VG(const function<void(const function<void(Graph &)> &)> &send_graphs,
       bool showp, bool warn_on_duplicates) {
  // set up uninitialized values
  init();
  show_progress = showp;
...
void VG::serialize(ostream &out) const {
  // we have to duplicate some functionality here to match the handle graph
  // interface, which is const this makes it hard to colocate path steps with
  // their nodes, so we don't really try to
  ...
  for_each_path_handle([&](const path_handle_t &path_handle) {
    ...
    // manually keep track of rank so we don't need to sync paths and can
    // therefor keep this const
    int32_t rank = 1;
...
void VG::deserialize(istream &in) { from_istream(in); }
...
size_t VG::get_length(const handle_t &handle) const {
  // Don't get the real sequence because it might need a reverse complement
  // calculation
...
bool VG::follow_edges_impl(
  ...
  if (found != edge_set.end()) {
    ...
    for (auto &id_and_flip : found->second) {
      // For each edge destination and the flag that says if we flip orientation
      // or not
...
bool VG::for_each_handle_impl(const function<bool(const handle_t &)> &iteratee,
                              bool parallel) const {
  if (parallel) {
...
size_t VG::get_node_count() const { return graph.node_size(); }

size_t VG::get_edge_count() const { return graph.edge_size(); }
...
string VG::get_subsequence(const handle_t &handle, size_t index,
                           size_t size) const {
  auto found = node_by_id.find(get_id(handle));
...
size_t VG::get_path_count() const { return paths._paths.size(); }

bool VG::for_each_path_handle_impl(
    const function<bool(const path_handle_t &)> &iteratee) const {
  return paths.for_each_name_stoppable(
      [&](const string &name) { return iteratee(get_path_handle(name)); });
}

handle_t VG::get_handle_of_step(const step_handle_t &step_handle) const {
  return get_handle(
      reinterpret_cast<const mapping_t *>(as_integers(step_handle)[1])
          ->node_id(),
      reinterpret_cast<const mapping_t *>(as_integers(step_handle)[1])
          ->is_reverse());
}
...
path_handle_t
VG::get_path_handle_of_step(const step_handle_t &step_handle) const {
...
step_handle_t VG::path_begin(const path_handle_t &path_handle) const {
  ...
  const auto &path_list =
      paths._paths.at(paths.get_path_name(as_integer(path_handle)));
  as_integers(step_handle)[1] = reinterpret_cast<int64_t>(
      path_list.empty() ? nullptr : &path_list.front());
  return step_handle;
}
...
step_handle_t VG::path_front_end(const path_handle_t &path_handle) const {
  // i'm a little uncomfortable using this value as a pointer sentinel, but i
  // think it'll only matter if there are 2^63 - 1 bytes, right?
  step_handle_t step_handle;
  as_integers(step_handle)[0] = as_integer(path_handle);
...
step_handle_t VG::get_next_step(const step_handle_t &step_handle) const {
  ...
  auto &path_list = paths._paths.at(
      paths.get_path_name(as_integer(get_path_handle_of_step(step_handle))));
  if (as_integers(step_handle)[1] == numeric_limits<uint64_t>::max()) {
    as_integers(next_step_handle)[1] =
        reinterpret_cast<int64_t>(&path_list.front());
  } else {
    auto iter =
        paths.mapping_itr
            .at(reinterpret_cast<mapping_t *>(as_integers(step_handle)[1]))
            .first;
    ++iter;
...
step_handle_t VG::get_previous_step(const step_handle_t &step_handle) const {
  ...
  if (as_integers(step_handle)[1] == reinterpret_cast<int64_t>(nullptr)) {
    as_integers(prev_step_handle)[1] =
        reinterpret_cast<int64_t>(&path_list.back());
  } else {
    auto iter =
        paths.mapping_itr
            .at(reinterpret_cast<mapping_t *>(as_integers(step_handle)[1]))
...
bool VG::has_next_step(const step_handle_t &step_handle) const {
  auto &path_list = paths._paths.at(
      paths.get_path_name(as_integer(get_path_handle_of_step(step_handle))));
  list<mapping_t>::const_iterator iter =
      paths.mapping_itr
          .at(reinterpret_cast<mapping_t *>(as_integers(step_handle)[1]))
          .first;
  iter++;
  return iter != path_list.end() ||
         (!path_list.empty() &&
...
bool VG::has_previous_step(const step_handle_t &step_handle) const {
  ...
  list<mapping_t>::const_iterator iter =
      paths.mapping_itr
          .at(reinterpret_cast<mapping_t *>(as_integers(step_handle)[1]))
          .first;
  return iter != path_list.begin() ||
         (!path_list.empty() &&
          get_is_circular(get_path_handle_of_step(step_handle)));
}

bool VG::for_each_step_on_handle_impl(
    const handle_t &handle,
    const function<bool(const step_handle_t &)> &iteratee) const {
  nid_t node_id = get_id(handle);
  if (paths.has_node_mapping(node_id)) {
    const map<int64_t, set<mapping_t *>> &node_mapping =
        paths.get_node_mapping(node_id);
...
handle_t VG::create_handle(const string &sequence, const nid_t &id) {
  if (id <= 0) {
    throw std::runtime_error(
        "error:[vg::VG] tried to create a node with non-positive ID " +
        std::to_string(id));
  }
...
vector<handle_t> VG::divide_handle(const handle_t &handle,
...
step_handle_t VG::append_step(const path_handle_t &path,
                              const handle_t &to_append) {
  // Make the new path mapping/visit (which weirdly requires the node length)
  paths.append_mapping(get_path_name(path), get_id(to_append),
                       get_is_reverse(to_append), get_length(to_append));
  // Make a handle for the step we just made, now last on the path.
  step_handle_t step;
  as_integers(step)[0] = as_integer(path);
  as_integers(step)[1] = reinterpret_cast<int64_t>(
      &paths._paths.at(paths.get_path_name(as_integer(path))).back());
  return step;
}

step_handle_t VG::prepend_step(const path_handle_t &path,
                               const handle_t &to_prepend) {
  // Make the new path mapping/visit (which weirdly requires the node length)
  paths.prepend_mapping(get_path_name(path), get_id(to_prepend),
                        get_is_reverse(to_prepend), get_length(to_prepend));
  // Make a handle for the step we just made, now last on the path.
  step_handle_t step;
  as_integers(step)[0] = as_integer(path);
  as_integers(step)[1] = reinterpret_cast<int64_t>(
      &paths._paths.at(paths.get_path_name(as_integer(path))).front());
  return step;
}

pair<step_handle_t, step_handle_t>
VG::rewrite_segment(const step_handle_t &segment_begin,
                    const step_handle_t &segment_end,
                    const vector<handle_t> &new_segment) {

  if (get_path_handle_of_step(segment_begin) !=
      get_path_handle_of_step(segment_end)) {
    cerr << "error:[VG] attempted to rewrite segment delimited by steps on two "
            "separate paths"
         << endl;
    exit(1);
  }
  ...
  // erase the old segment, using the get_next_step logic to wrap around
  // circular paths

  // collect the mapping_t*'s that we'll need to erase from the mapping_itr once
  // we don't need them for get_next_step
  ...
  auto &path_list = paths._paths.at(
      paths.get_path_name(as_integer(get_path_handle_of_step(segment_begin))));
  ...
  if (reinterpret_cast<mapping_t *>(as_integers(segment_end)[1]) != nullptr) {
    last_pos =
        paths.mapping_itr
            .at(reinterpret_cast<mapping_t *>(as_integers(segment_end)[1]))
            .first;
    ;
  } else {
  ...
  // init the range we'll return, the past-the-last position of which shouldn't
  // change from the input
  ...
  for (size_t i = 0; i < new_segment.size(); ++i) {
    ...
    // TODO: there's no efficient way to maintain the ranks dynamically if we're
    // going to be expanding them
    ...
    if (new_segment.size() <= ranks.size()) {
      paths.mappings_by_rank[get_path_name(as_path_handle(path_id))][ranks[i]] =
          &(*iterator);
    }

    paths.mapping_itr[&(*iterator)] = pair<list<mapping_t>::iterator, int64_t>(
        iterator, as_integers(segment_end)[0]);
    paths.node_mapping[get_id(handle)][path_id].insert(&(*iterator));

    // on the first iteration, construct the first step handle for the return
    // value
    if (first_iter) {
      as_integers(return_val.first)[1] =
          reinterpret_cast<int64_t>(&(*iterator));
      first_iter = false;
    }
  }
  ...
  for (size_t i = new_segment.size(); i < ranks.size(); ++i) {
    paths.mappings_by_rank[get_path_name(as_path_handle(path_id))].erase(
        ranks[i]);
  }
...
void VG::serialize_to_function(const function<void(Graph &)> &emit,
                               nid_t chunk_size) {
  ...
  for (size_t element_start = 0; element_start < graph.node_size();
       element_start += chunk_size) {
    ...
    for (size_t j = element_start;
         j < element_start + chunk_size && j < graph.node_size(); ++j) {
    ...
    for (auto &p : sorted_paths) {
      ...
      // May not be contiguous because other chunks may contain nodes between
      // the nodes in this one
      for (auto &m : path) {
...
void VG::serialize_to_emitter(vg::io::ProtobufEmitter<Graph> &emitter,
                              nid_t chunk_size) {
  // Serialize and make the emitter write every chunk
  serialize_to_function(
      [&emitter](const Graph &chunk) { emitter.write_copy(chunk); },
      chunk_size);
}

void VG::serialize_to_ostream(ostream &out, nid_t chunk_size) {
  // Make an emitter that serializes each chunk as its own group, like we did
  // before using emitters. This is good for indexing.
...
void VG::add_node(const Node &node) {
  if (!has_node(node)) {
    ...
    *new_node = node; // overwrite it with the value of the given node
    node_by_id[new_node->id()] =
        new_node; // and insert into our id lookup table
...
size_t VG::node_count(void) const { return graph.node_size(); }

size_t VG::edge_count(void) const { return graph.edge_size(); }
...
int VG::start_degree(Node *node) { return edges_start(node).size(); }

int VG::end_degree(Node *node) { return edges_end(node).size(); }
...
void VG::edges_of_node(Node *node, vector<Edge *> &edges) {
  for (pair<nid_t, bool> &off_start : edges_start(node)) {
    ...
    Edge *edge =
        edge_by_sides[NodeSide::pair_from_start_edge(node->id(), off_start)];
    if (!edge) {
      cerr << "error:[VG::edges_of_node] nonexistent start edge "
           << off_start.first << " start <-> " << node->id()
           << (off_start.second ? " start" : " end") << endl;
      exit(1);
    }
  ...
  for (pair<nid_t, bool> &off_end : edges_end(node)) {
    // And on its end
    Edge *edge =
        edge_by_sides[NodeSide::pair_from_end_edge(node->id(), off_end)];
    if (!edge) {
      cerr << "error:[VG::edges_of_node] nonexistent end edge " << off_end.first
           << " end <-> " << node->id() << (off_end.second ? " end" : " start")
           << endl;
      exit(1);
    }
...
set<pair<NodeSide, bool>> VG::sides_context(nid_t node_id) {
  // return the side we're going to and if we go from the start or end to get
  // there
...
bool VG::is_ancestor_prev(nid_t node_id, nid_t candidate_id, set<nid_t> &seen,
                          size_t steps) {
  ...
  for (auto &side : sides_to(NodeSide(node_id, false))) {
    if (seen.count(side.node))
      continue;
    seen.insert(side.node);
    if (is_ancestor_prev(side.node, candidate_id, seen, steps - 1))
      return true;
  }
  return false;
}
...
bool VG::is_ancestor_next(nid_t node_id, nid_t candidate_id, set<nid_t> &seen,
                          size_t steps) {
  if (node_id == candidate_id)
    return true;
  if (!steps)
  ...
  for (auto &side : sides_from(NodeSide(node_id, true))) {
    if (seen.count(side.node))
      continue;
    seen.insert(side.node);
    if (is_ancestor_next(side.node, candidate_id, seen, steps - 1))
      return true;
  }
  return false;
}
...
nid_t VG::common_ancestor_prev(nid_t id1, nid_t id2, size_t steps) {
  ...
  auto scan = [this](nid_t id1, nid_t id2, size_t steps) -> nid_t {
    ...
    for (size_t i = 0; i < steps; ++i) {
      ...
      if (to_visit.empty())
        return -1; // we hit the end of the graph
    }
    return 0;
  };
...
nid_t VG::common_ancestor_next(nid_t id1, nid_t id2, size_t steps) {
  // arbitrarily step forward from node 1 asking if we are next-ancestral to
  // node 2
  auto scan = [this](nid_t id1, nid_t id2, size_t steps) -> nid_t {
    set<nid_t> to_visit;
    to_visit.insert(id1);
...
void VG::expand_context(VG &g, size_t distance, bool add_paths,
                        bool use_steps) {
...
void VG::expand_context_by_steps(VG &g, size_t steps, bool add_paths) {
  ...
  // and expand
  for (size_t i = 0; i < steps; ++i) {
    // break if we have completed the (sub)graph accessible from our starting
    // graph
    if (to_visit.empty())
      break;
    set<nid_t> to_visit_next;
...
void VG::expand_context_by_length(VG &g, size_t length, bool add_paths,
                                  bool reflect, const set<NodeSide> &barriers) {
  ...
  while (!active.empty()) {
    ...
    if (!barriers.count(here)) {
      ...
      for (auto connected : sides_of(here)) {
...
#ifdef debug
        cerr << "\tTake node " << connected.node << " size "
             << get_node(connected.node)->sequence().size() << endl;
#endif
        ...
        // Deduct the length of the reached node from the budget of this
        // NodeSide
        int64_t new_budget =
            budget - get_node(connected.node)->sequence().size();
...
bool VG::adjacent(const Position &pos1, const Position &pos2) {
  ...
  if (pos1.node_id() == pos2.node_id()) {
    ...
    if (pos1.offset() == node1->sequence().size() - 1 && pos2.offset() == 0) {
...
set<Edge *> VG::get_path_edges(void) {
  ...
  function<void(const Path &)> lambda = [this, &edges](const Path &path) {
    for (size_t i = 1; i < path.mapping_size(); ++i) {
      ...
      if (!adjacent_mappings(m1, m2))
        continue; // the path is completely represented here
      auto s1 = NodeSide(m1.position().node_id(),
                         (m1.position().is_reverse() ? false : true));
      auto s2 = NodeSide(m2.position().node_id(),
                         (m2.position().is_reverse() ? true : false));
      // check that we always have an edge between the two nodes in the correct
      // direction
      if (has_edge(s1, s2)) {
    ...
    if (path.is_circular()) {
      ...
      // if (!adjacent_mappings(m1, m2)) continue; // the path is completely
      // represented here
      auto s1 = NodeSide(m1.position().node_id(),
                         (m1.position().is_reverse() ? false : true));
      auto s2 = NodeSide(m2.position().node_id(),
...
// true if the mapping completely covers the node it maps to and is a perfect
// match
bool VG::mapping_is_total_match(const Mapping &m) {
  return mapping_is_simple_match(m) &&
         mapping_from_length(m) ==
             get_node(m.position().node_id())->sequence().size();
}
...
bool VG::has_node(const Node &node) const { return has_node(node.id()); }
...
bool VG::has_edge(Edge *edge) const { return edge && has_edge(*edge); }

bool VG::has_edge(const Edge &edge) const {
  return edge_by_sides.find(NodeSide::pair_from_edge(edge)) !=
         edge_by_sides.end();
}
...
bool VG::has_inverting_edge(Node *n) {
  for (auto e : edges_of(n)) {
    if ((e->from_start() || e->to_end()) && !(e->from_start() && e->to_end())) {
      return true;
    }
  }
  return false;
...
bool VG::has_inverting_edge_from(Node *n) {
  for (auto e : edges_of(n)) {
    if (e->from() == n->id() && (e->from_start() || e->to_end()) &&
...
bool VG::has_inverting_edge_to(Node *n) {
  for (auto e : edges_of(n)) {
    if (e->to() == n->id() && (e->from_start() || e->to_end()) &&
        !(e->from_start() && e->to_end())) {
      return true;
    }
  }
...
void VG::remove_duplicates(void) {
  ...
  for (size_t i = 0; i < graph.node_size(); ++i) {
    ...
    if (f != node_counts.end() && f->second > 1) {
      --f->second;
      nodes_to_destroy.push_back(n);
    }
  }
  ...
  for (nid_t i = 0; i < graph.edge_size(); ++i) {
    ...
    if (f != edge_counts.end() && f->second > 1) {
      --f->second;
      edges_to_destroy.push_back(e);
    }
  }
...
void VG::merge(VG &g) { merge(g.graph); }
...
void VG::extend(const VG &g, bool warn_on_duplicates) {
  for (nid_t i = 0; i < g.graph.node_size(); ++i) {
    ...
    if (n.id() == 0) {
      ...
      cerr << "[vg] warning: node ID " << n.id()
           << " appears multiple times. Skipping." << endl;
    }
  }
  for (nid_t i = 0; i < g.graph.edge_size(); ++i) {
    ...
    if (!has_edge(e)) {
      ...
      cerr << "[vg] warning: edge " << e.from()
           << (e.from_start() ? " start" : " end") << " <-> " << e.to()
           << (e.to_end() ? " end" : " start")
           << " appears multiple times. Skipping." << endl;
    }
  }
...
// extend this graph by g, connecting the tails of this graph to the heads of
// the other the ids of the second graph are modified for compact representation
void VG::append(VG &g) {

  // compact and increment the ids of g out of range of this graph
  // g.compact_ids();

  // assume we've already compacted the other, or that id compaction doesn't
  // matter just get out of the way
  ...
  // note that we don't use merge_union because we are ensured non-overlapping
  // ids
  merge(g);

  /*
  cerr << "this graph size " << node_count() << " nodes " << edge_count() << "
  edges" << endl; cerr << "in append with " << heads.size() << " heads and " <<
  tails.size() << " tails" << endl;
  */
  ...
  // and join paths that are embedded in the graph, where path names are the
  // same
...
void VG::include(const Path &path) {
  for (size_t i = 0; i < path.mapping_size(); ++i) {
    if (!mapping_is_simple_match(path.mapping(i))) {
      cerr
          << "mapping " << pb2json(path.mapping(i))
          << " cannot be included in the graph because it is not a simple match"
          << endl;
      // exit(1);
    }
  }
...
void VG::reassign_node_ids(
    const std::function<nid_t(const nid_t &)> &get_new_id) {
  // #pragma omp parallel for
  for_each_node([&get_new_id](Node *n) { n->set_id(get_new_id(n->id())); });
  // #pragma omp parallel for
...
void VG::compact_ids(hash_map<nid_t, nid_t> &new_id) {
  ...
  for_each_node([&id, &new_id](Node *n) { new_id[n->id()] = id++; });
...
void VG::increment_node_ids(nid_t increment) {
  for_each_node_parallel(
      [increment](Node *n) { n->set_id(n->id() + increment); });
...
void VG::decrement_node_ids(nid_t decrement) { increment_node_ids(-decrement); }
...
void VG::swap_node_id(Node *node, nid_t new_id) {
  ...
  // we check if the old node exists, and bail out if we're not doing what we
  // expect
  ...
  auto fix_edge = [&](Edge *edge) {
    // Destroy that edge
    edges_to_destroy.emplace(NodeSide(edge->from(), !edge->from_start()),
                             NodeSide(edge->to(), edge->to_end()));

    // Make a new edge with our new ID as from or to (or both), depending on
    // which it was before.
    // TODO: Is there a cleaner way to do this?
    if (edge->from() == old_id) {
      if (edge->to() == old_id) {
        edges_to_create.emplace(NodeSide(new_id, !edge->from_start()),
                                NodeSide(new_id, edge->to_end()));
      } else {
        edges_to_create.emplace(NodeSide(new_id, !edge->from_start()),
                                NodeSide(edge->to(), edge->to_end()));
      }
    } else {
      edges_to_create.emplace(NodeSide(edge->from(), !edge->from_start()),
                              NodeSide(new_id, edge->to_end()));
    }
  };
  ...
  for (pair<nid_t, bool> &other : edges_start(old_id)) {
    // Get the actual Edge
    // We're at a start, so we go to the end of the other node normally, and the
    // start if the other node is backward
    Edge *edge = edge_by_sides[minmax(NodeSide(old_id, false),
                                      NodeSide(other.first, !other.second))];
  ...
  for (pair<nid_t, bool> &other : edges_end(old_id)) {
    // Get the actual Edge
    // We're at an end, so we go to the start of the other node normally, and
    // the end if the other node is backward
    Edge *edge = edge_by_sides[minmax(NodeSide(old_id, true),
                                      NodeSide(other.first, other.second))];
...
bool VG::apply_ordering(const vector<handle_t> &ordering, bool compact_ids) {
  ...
  if (get_node_count() != ordering.size()) {
    cerr << "error:[algorithms] attempting to sort a graph with an incomplete "
            "ordering"
         << endl;
    exit(1);
  }

  // TODO: we don't check that all nodes are present only once, which might be
  // nice to do
  ...
  for_each_handle([&](const handle_t &at_index) {
    ...
    // Swap the handle we observe at this index with the handle that we know
    // belongs at this index. The loop invariant is that all the handles before
    // index are the correct sorted handles in the right order. Note that this
    // ignores orientation
...
map<nid_t, vcflib::Variant>
VG::get_node_nid_to_variant(vcflib::VariantCallFile vfile) {
...
static void triple_to_vg(void *user_data, raptor_statement *triple) {
  ...
  const string sub(
      reinterpret_cast<char *>(raptor_term_to_string(triple->subject)));
  const string pred(
      reinterpret_cast<char *>(raptor_term_to_string(triple->predicate)));
  const string obj(
      reinterpret_cast<char *>(raptor_term_to_string(triple->object)));
  ...
  if (pred == (vg_node_p) || reverse) {
    ...
    const string pathname =
        sub.substr(base_uri.length() + 6, sub.length() - sub.find_last_of('-'));

    // TODO we are using a nasty trick here, which needs to be fixed.
    // We are using knowledge about the uri format to determine the rank of the
    // step.
#ifdef debug
#endif
    try {
      ...
      cerr << "[vg view] assumption about rdf structure was wrong, parsing "
              "failed"
           << endl;
      exit(1);
    }
...
void VG::from_turtle(string filename, string base_uri, bool showp) {
  ...
  if (!world) {
    cerr
        << "[vg view] we could not start the rdf environment needed for parsing"
        << endl;
    exit(1);
  }
  ...
  std::tuple<VG *, Paths *, string> user_data =
      make_tuple(this, paths, base_uri);
  ...
  uri_base = raptor_new_uri(
      world, reinterpret_cast<const unsigned char *>(base_uri.c_str()));
  ...
  paths->for_each([this](const Path &path) { this->include(path); });
}
...
// depth first search across node traversals with interface to traversal tree
// via callback
void VG::dfs(
    const function<void(NodeTraversal)>
        &node_begin_fn, // called when node orientation is first encountered
    const function<void(NodeTraversal)>
    ...
    const function<void(Edge *)>
        &tree_fn, // called when an edge forms part of the DFS spanning tree
    const function<void(Edge *)> &edge_curr_fn, // called when we meet an edge
                                                // in the current tree component
    const function<void(Edge *)>
        &edge_cross_fn, // called when we meet an edge in an already-traversed
                        // tree component
    const vector<NodeTraversal> *sources, // start only at these node traversals
    const unordered_set<NodeTraversal>
        *sinks // when hitting a sink, don't keep walking
) {

  // to maintain search state
  enum SearchState { PRE = 0, CURR, POST };
  unordered_map<NodeTraversal, SearchState>
      state; // implicitly constructed entries will be PRE.

  // to maintain stack frames
  struct Frame {
    NodeTraversal trav;
    vector<Edge *>::iterator begin, end;
    Frame(NodeTraversal t, vector<Edge *>::iterator b,
  ...
  // do dfs from given root.  returns true if terminated via break condition,
  // false otherwise
  function<bool(NodeTraversal &)> dfs_single_source = [&](NodeTraversal &root) {
    ...
    while (!todo.empty()) {
      ...
      while (edges_begin != edges_end) {
        ...
        } else if (edge->to() == trav.node->id() &&
                   edge->from() != trav.node->id()) {
...
void VG::dfs(const function<void(NodeTraversal)> &node_begin_fn,
  ...
  dfs(
      node_begin_fn, node_end_fn, [](void) { return false; }, edge_noop,
      edge_noop, edge_noop, edge_noop, sources, sinks);
}
...
void VG::dfs(const function<void(NodeTraversal)> &node_begin_fn,
  ...
  auto edge_noop = [](Edge *e) {};
  dfs(node_begin_fn, node_end_fn, break_fn, edge_noop, edge_noop, edge_noop,
      edge_noop, NULL, NULL);
}

// returns the rank of the node in the protobuf array that backs the graph
int VG::node_rank(Node *node) { return node_index[node]; }

// returns the rank of the node in the protobuf array that backs the graph
int VG::node_rank(nid_t id) { return node_index[get_node(id)]; }
...
VG VG::reverse_complement_graph(
    unordered_map<nid_t, pair<nid_t, bool>> &node_translation) {
  nid_t max_id = 0;
  VG rev_comp;
  for (size_t i = 0; i < graph.node_size(); i++) {
...
size_t VG::size(void) { return graph.node_size(); }

size_t VG::length(void) { return get_total_length(); }
...
Edge *VG::create_edge(NodeTraversal left, NodeTraversal right) {
  // Connect to the start of the left node if it is backward, and the end of the
  // right node if it is backward.
  return create_edge(left.node->id(), right.node->id(), left.backward,
                     right.backward);
}

Edge *VG::create_edge(NodeSide side1, NodeSide side2) {
  // Connect to node 1 (from start if the first side isn't an end) to node 2 (to
  // end if the second side is an end)
...
Edge *VG::get_edge(const NodeTraversal &left, const NodeTraversal &right) {
  ...
  // We used the end of left if if isn't backward, and we used the end of right
  // if it is.
...
void VG::destroy_edge(Edge *edge) {
  ...
  // that if we call erase twice on the same entry, we'll end up corrupting the
  // hash_map
  //
...
void VG::unindex_edge_by_node_sides(const NodeSide &side1,
...
void VG::unindex_edge_by_node_sides(Edge *edge) {
  ...
  // cerr << "Unindexing edge " << edge_pair.first << "<-> " << edge_pair.second
  // << endl;
  ...
  if (edge->from_start()) {
    ...
    if (edges_on_start[edge->from()].empty())
      edges_on_start.erase(edge->from());

    // cerr << "Removed " << edge->from() << "-start to " << edge->to() << "
    // orientation " << relative_orientation << endl;
  } else {
    // The edge is on the end of the from node, do remove it from the end of the
    // from node.
    std::pair<nid_t, bool> to_remove{edge->to(), relative_orientation};
    swap_remove(edges_end(edge->from()), to_remove);
    if (edges_on_end[edge->from()].empty())
      edges_on_end.erase(edge->from());

    // cerr << "Removed " << edge->from() << "-end to " << edge->to() << "
    // orientation " << relative_orientation << endl;
  }

  if (edge->from() != edge->to() || edge->from_start() == edge->to_end()) {
    // Same for the to node, if we aren't just on the same node and side as with
    // the from node.
    if (edge->to_end()) {
      std::pair<nid_t, bool> to_remove{edge->from(), relative_orientation};
      swap_remove(edges_end(edge->to()), to_remove);
      if (edges_on_end[edge->to()].empty())
        edges_on_end.erase(edge->to());

      // cerr << "Removed " << edge->to() << "-end to " << edge->from() << "
      // orientation " << relative_orientation << endl;
    } else {
      std::pair<nid_t, bool> to_remove{edge->from(), relative_orientation};
      swap_remove(edges_start(edge->to()), to_remove);
      if (edges_on_start[edge->to()].empty())
        edges_on_start.erase(edge->to());

      // cerr << "Removed " << edge->to() << "-start to " << edge->from() << "
      // orientation "
...
void VG::index_edge_by_node_sides(Edge *edge) {
  ...
  if (edge->from() != edge->to() || edge->from_start() == edge->to_end()) {
    // Only index the other end of the edge if the edge isn't a self-loop on a
    // single side.
    if (edge->to_end()) {
      edges_on_end[edge->to()].emplace_back(edge->from(), relative_orientation);
    } else {
      edges_on_start[edge->to()].emplace_back(edge->from(),
...
Node *VG::create_node(const string &seq, nid_t id) {
  // We no longer support a 0 value as a sentinel to represent letting the graph
  // assign the ID. It was too easy to accidentally pass 0 by forgetting to
  // offset an incoming source of IDs by 1. Use the overload without an ID
  // instead.
  assert(id != 0);
  // create the node
...
void VG::for_each_connected_node(Node *node, function<void(Node *)> lambda) {
  ...
  while (!to_visit.empty()) {
    ...
    for (Edge *edge : edges) {
      if (edge->from() != visiting->id() &&
          !visited.count(get_node(edge->from()))) {
        // We found a new node on the from of the edge
        to_visit.insert(get_node(edge->from()));
      } else if (edge->to() != visiting->id() &&
                 !visited.count(get_node(edge->to()))) {
...
// a graph composed of this node and the edges that can be uniquely assigned to
// it
void VG::nonoverlapping_node_context_without_paths(Node *node, VG &g) {
  ...
  auto grab_edge = [&](Edge *e) {
    ...
    if (node->id() == owner_id || !has_node(owner_id)) {
      // Either we are the owner, or the owner isn't in the graph to get
      // serialized.
      g.add_edge(*e);
    }
  };
...
void VG::destroy_node(nid_t id) { destroy_node(get_node(id)); }

void VG::destroy_node(Node *node) {
  // if (!is_valid()) cerr << "graph is invalid before destroy_node" << endl;
  // cerr << "destroying node " << node->id() << " degrees " <<
  // start_degree(node) << ", " << end_degree(node) << endl;
  ...
  for (auto &other_end : edges_start(node)) {
    ...
    edges_to_destroy.insert(
        NodeSide::pair_from_start_edge(node->id(), other_end));
  }

  for (auto &other_end : edges_end(node)) {
    // Destroy all the edges on its end
    edges_to_destroy.insert(
        NodeSide::pair_from_end_edge(node->id(), other_end));
  }
  ...
  // remove this node (which is now the last one) and remove references from the
  // indexes
...
void VG::remove_null_nodes(void) {
  ...
  for (vector<nid_t>::iterator n = to_remove.begin(); n != to_remove.end();
       ++n) {
    destroy_node(*n);
  }
}
...
void VG::remove_null_nodes_forwarding_edges(void) {
  ...
  for (vector<Node *>::iterator n = to_remove.begin(); n != to_remove.end();
       ++n, ++i) {
...
void VG::remove_node_forwarding_edges(Node *node) {
  ...
  // Grab all the nodes attached to our start, with true if the edge goes to
  // their start
  vector<pair<nid_t, bool>> &start = edges_start(node);
  // Grab all the nodes attached to our end, with true if the edge goes to their
  // end
...
void VG::divide_node(Node *node, vector<int> &positions,
                     vector<Node *> &parts) {
...
#pragma omp critical(cerr)
      {
        ...
        cerr << " -- position (" << pos
             << ") is less than 0 or greater than sequence length ("
  ...
  int last_pos = 0;
  for (auto pos : positions) {
    // Make all the nodes ending at the given positions, grabbing the
    // appropriate substrings
    Node *new_node =
        create_node(node->sequence().substr(last_pos, pos - last_pos));
    last_pos = pos;
...
#pragma omp critical(cerr)
  ...
  // Create edges between the left node (optionally its start) and the right
  // node (optionally its end)
  set<pair<pair<nid_t, bool>, pair<nid_t, bool>>> edges_to_create;

  // replace the connections to the node's start
  for (auto e : edges_start(node)) {
    // We have to check for self loops, as these will be clobbered by the
    // division of the node
    if (e.first == node->id()) {
    ...
    edges_to_create.emplace(make_pair(e.first, e.second),
                            make_pair(parts.front()->id(), false));
  }
  ...
  for (auto e : edges_end(node)) {
    ...
    edges_to_create.emplace(make_pair(parts.back()->id(), false),
                            make_pair(e.first, e.second));
  }
  ...
  for (int i = 0; i < parts.size() - 1; i++) {
    // Connect all the new parts left to right. These edges always go from end
    // to start.
...
#pragma omp critical(cerr)
      ...
      // We can only work on full-length perfect matches, because we make the
      // cuts in mapping space.
      ...
      for (int i = 0; i < positions.size(); i++) {
        ...
        // Note that we are cutting at mapping-relative locations and not
        // node-relative locations.
...
void VG::divide_path(map<long, nid_t> &path, long pos, Node *&left,
                     Node *&right) {
  ...
  // nothing to do
  if (node_pos == pos) {
    map<long, nid_t>::iterator n = target;
    --n;
...
set<NodeTraversal> VG::travs_of(NodeTraversal node) {
  ...
  std::set_union(tos.begin(), tos.end(), froms.begin(), froms.end(),
...
void VG::nodes_prev(NodeTraversal node, vector<NodeTraversal> &nodes) {
  ...
  vector<pair<nid_t, bool>> &left_nodes =
      node.backward ? edges_end(node.node) : edges_start(node.node);
  for (auto &prev : left_nodes) {
    // Make a NodeTraversal that is an oriented description of the node attached
    // to our relative left. If we're backward, and it's in the same relative
    // orientation as us, it needs to be backward too.
...
void VG::nodes_next(NodeTraversal node, vector<NodeTraversal> &nodes) {
  ...
  vector<pair<nid_t, bool>> &right_nodes =
      node.backward ? edges_start(node.node) : edges_end(node.node);
  for (auto &next : right_nodes) {
    // Make a NodeTraversal that is an oriented description of the node attached
    // to our relative right. If we're backward, and it's in the same relative
    // orientation as us, it needs to be backward too.
...
// path utilities
// these are in this class because attributes of the path (such as its sequence)
// are a property of the graph
...
Path VG::create_path(const list<NodeTraversal> &nodes) {
  ...
  for (const NodeTraversal &n : nodes) {
    ...
    if (n.backward)
      mapping->mutable_position()->set_is_reverse(n.backward);
    // TODO: Is it OK if we say we're at a mapping at offset 0 of this node,
    // backwards? Or should we offset ourselves to the end?
  }
  return path;
}

void VG::expand_path(const list<NodeTraversal> &path,
                     vector<NodeTraversal> &expanded) {
  for (list<NodeTraversal>::const_iterator n = path.begin(); n != path.end();
       ++n) {
...
void VG::expand_path(list<NodeTraversal> &path,
...
void VG::edit(vector<Path> &paths_to_add, vector<Translation> *out_translations,
              bool save_paths, bool update_paths, bool break_at_ends) {
  ...
  // If we are going to actually add the paths to the graph, we need to break at
  // path ends
  ...
  // we remember the sequences of nodes we've added at particular positions on
  // the forward strand
  unordered_map<pair<pos_t, string>, vector<nid_t>> added_seqs;
  // we will record the nodes that we add, so we can correctly make the returned
  // translation
  ...
  for (auto &path : simplified_paths) {
    ...
    Path added = add_nodes_and_edges(this, path, node_translation, added_seqs,
                                     added_nodes, orig_node_sizes);

    if (save_paths) {
      // Add this path to the graph's paths object without rebuilding path
      // ranks, aux mapping, etc.
      paths.extend(added, false, false);
    }

    if (update_paths) {
      // Replace the simplified path in original graph space with one in new
      // graph space.
      path = added;
    }
  }
  ...
  if (update_paths) {
    // We replaced all the paths in simplifies_paths, so send those back out as
  ...
  if (out_translations != nullptr) {
    *out_translations =
        make_translation(this, node_translation, added_nodes, orig_node_sizes);
  }
}

// Streaming edit will use much less memory than the older version (above), at
// the cost of needing to do multiple passes over the input paths.
void VG::edit(const string &paths_to_add_path,
              vector<Translation> *out_translations, bool save_paths,
              const string &out_gam_path, bool break_at_ends,
  ...
  augment(this, paths_to_add_path, "GAM", out_translations, out_gam_path,
          save_paths, break_at_ends, remove_softclips);
}

// The not quite as robust (TODO: how?) but actually efficient way to edit the
// graph.
vector<Translation> VG::edit_fast(const Path &path, set<NodeSide> &dangling,
  ...
  for (auto &kv : node_translation) {
    cerr << "Translate old " << kv.first << " to "
         << (kv.second == nullptr ? (nid_t)0 : (nid_t)kv.second->id()) << endl;
  }
#endif
  ...
  unordered_map<pair<pos_t, string>, vector<nid_t>> added_seqs;
  // we will record the nodes that we add, so we can correctly make the returned
  // translation for novel insert nodes
  unordered_map<nid_t, Path> added_nodes;
  // create new nodes/wire things up.
  add_nodes_and_edges(this, path, node_translation, added_seqs, added_nodes,
                      orig_node_sizes, dangling, max_node_size);
  ...
  for (auto &kv : node_translation) {
    // For every translation from an old position to a new node (which may be
    // null)
    if (kv.second == 0) {
    ...
    // as the old node.
    *(to_mapping->mutable_position()) =
        make_position(kv.second, is_rev(kv.first), 0);
  ...
  // TODO: maybe we should also add translations to anchor completely novel
  // nodes?
...
Alignment VG::random_read(size_t read_len, mt19937 &rng, nid_t min_id,
                          nid_t max_id, bool either_strand) {
  ...
  while (read.size() < read_len) {
    ...
    string addition = (node.backward ? reverse_complement(node.node->sequence())
                                     : node.node->sequence());
  ...
  if (either_strand && binary_dist(rng) == 1) {
    ...
    aln = reverse_complement_alignment(
        aln, (function<nid_t(nid_t)>)([this](nid_t id) {
...
bool VG::is_valid(bool check_nodes, bool check_edges, bool check_paths,
                  bool check_orphans) {

  if (check_nodes) {

    if (node_by_id.size() != graph.node_size()) {
      cerr << "graph invalid: node count is not equal to that found in node "
              "by-id index"
           << endl;
      return false;
    }
    ...
    for (int i = 0; i < graph.node_size(); ++i) {
      ...
      if (node_by_id.find(n->id()) == node_by_id.end()) {
        cerr << "graph invalid: node " << n->id() << " missing from by-id index"
             << endl;
        return false;
      }
    }
  ...
  if (check_edges) {
    for (int i = 0; i < graph.edge_size(); ++i) {
      ...
      if (node_by_id.find(f) == node_by_id.end()) {
        cerr << "graph invalid: edge index=" << i << " (" << f << "->" << t
             << ") cannot find node (from) " << f << endl;
        return false;
      }
      if (node_by_id.find(t) == node_by_id.end()) {
        cerr << "graph invalid: edge index=" << i << " (" << f << "->" << t
             << ") cannot find node (to) " << t << endl;
        return false;
      }
      ...
      if (!edges_on_start.count(f) && !edges_on_end.count(f)) {
        ...
        cerr << "graph invalid: edge index=" << i
             << " could not find entry in either index for 'from' node " << f
             << endl;
        return false;
      }
      ...
      if (!edges_on_start.count(t) && !edges_on_end.count(t)) {
        // todo check if it's in the vector
        cerr << "graph invalid: edge index=" << i
             << " could not find entry in either index for 'to' node " << t
             << endl;
        return false;
      }
    }

    for (pair<const nid_t, vector<pair<nid_t, bool>>> &start_and_edges :
         edges_on_start) {
      for (auto &edge_destination : start_and_edges.second) {
        // We're on the start, so we go to the end if we aren't a reversing edge
        Edge *e = get_edge(NodeSide::pair_from_start_edge(start_and_edges.first,
                                                          edge_destination));
        if (!e) {
          cerr << "graph invalid, edge is null" << endl;
          return false;
        }
        if (start_and_edges.first != e->to() &&
            start_and_edges.first != e->from()) {
          // It needs to be attached to the node we looked up
          cerr << "graph invalid: edge " << e->from() << "->" << e->to()
               << " doesn't have start-indexed node in "
               << start_and_edges.first << "<->" << edge_destination.first
               << endl;
          return false;
        }
        if (edge_destination.first != e->to() &&
            edge_destination.first != e->from()) {
          // It also needs to be attached to the node it says it goes to
          cerr << "graph invalid: edge " << e->from() << "->" << e->to()
               << " doesn't have non-start-indexed node in "
               << start_and_edges.first << "<->" << edge_destination.first
               << endl;
          return false;
        }
        if (!((start_and_edges.first == e->to() && !e->to_end()) ||
          ...
          // The edge needs to actually attach to the start of the node we
          // looked it up for. So at least one of its ends has to be to the
          // start of the correct node. It may also be attached to the end.
          cerr << "graph invalid: edge " << e->from() << "->" << e->to()
               << " doesn't attach to start of " << start_and_edges.first
               << endl;
          return false;
        }
        if (!has_node(e->from())) {
          cerr << "graph invalid: edge from a non-existent node " << e->from()
               << "->" << e->to() << endl;
          return false;
        }
        if (!has_node(e->to())) {
          cerr << "graph invalid: edge to a non-existent node " << e->from()
               << "->" << e->to() << endl;
          return false;
        }
      }
    ...
    for (pair<const nid_t, vector<pair<nid_t, bool>>> &end_and_edges :
         edges_on_end) {
      for (auto &edge_destination : end_and_edges.second) {
        Edge *e = get_edge(NodeSide::pair_from_end_edge(end_and_edges.first,
                                                        edge_destination));
        if (!e) {
          cerr << "graph invalid, edge is null" << endl;
          return false;
        }
        if (end_and_edges.first != e->to() &&
            end_and_edges.first != e->from()) {
          // It needs to be attached to the node we looked up
          cerr << "graph invalid: edge " << e->from() << "->" << e->to()
        ...
        if (edge_destination.first != e->to() &&
          ...
          // It also needs to be attached to the node it says it goes to
          cerr << "graph invalid: edge " << e->from() << "->" << e->to()
               << " doesn't have non-end-indexed node in "
               << end_and_edges.first << "<->" << edge_destination.first
               << endl;
          return false;
        }
        if (!((end_and_edges.first == e->to() && e->to_end()) ||
          ...
          // The edge needs to actually attach to the end of the node we looked
          // it up for. So at least one of its ends has to be to the end of the
          // correct node. It may also be attached to the start.
          cerr << "graph invalid: edge " << e->from() << "->" << e->to()
  ...
  if (check_paths) {
    bool paths_ok = true;
    function<void(const Path &)> lambda = [this, &paths_ok](const Path &path) {
      if (!paths_ok) {
        return;
      }
      ...
      for (size_t i = 1; i < path.mapping_size(); ++i) {
        ...
        auto s2 = NodeSide(m2.position().node_id(),
                           (m2.position().is_reverse() ? true : false));
        // check that we always have an edge between the two nodes in the
        // correct direction
        if (!has_edge(s1, s2)) {
          cerr << "graph path '" << path.name() << "' invalid: edge from " << s1
               << " to " << s2 << " does not exist" << endl;
          paths_ok = false;
          // return;;
        }

        // in the four cases below, we check that edges always incident the tips
        // of nodes when edit length, offsets and strand flipping of mappings
        // are taken into account:

        // NOTE: Because of the !adjacent_mappings check above, mappings that
        // are out of order
        //       will be ignored.  If they are invalid, it won't be caught.
        //       Solution is to sort by rank, but I'm not sure if any of this is
        //       by design or not...

        auto &p1 = m1.position();
        if (!has_node(p1.node_id())) {
          cerr << "graph path '" << path.name() << "' has invalid mapping "
               << pb2json(m1) << ": node does not exist" << endl;
          paths_ok = false;
          return;
        }
        ...
        if (!has_node(p2.node_id())) {
          cerr << "graph path '" << path.name() << "' has invalid mapping "
               << pb2json(m2) << ": node does not exist" << endl;
          paths_ok = false;
          return;
        }
        ...
        if (p1.offset() + m1_edit_length != n1.sequence().length()) {
          cerr << "graph path '" << path.name() << "' has invalid mapping "
               << pb2json(m1) << ": offset (" << p1.offset()
               << ") + from_length (" << m1_edit_length << ")"
        ...
        if (p2.offset() > 0) {
          cerr << "graph path '" << path.name() << "' has invalid mapping "
               << pb2json(m2) << ": offset=" << p2.offset()
               << " found when offset=0 expected" << endl;
          paths_ok = false;
          return;
        }
      }
...
void VG::to_dot(ostream &out, vector<Alignment> alignments, vector<Locus> loci,
                bool show_paths, bool walk_paths, bool annotate_paths,
                bool show_mappings, bool simple_mode, bool noseq_mode,
                bool invert_edge_ports, bool color_variants,
                bool ultrabubble_labeling, bool skip_missing_nodes,
                bool ascii_labels, int random_seed) {
  ...
  if (ultrabubble_labeling) {
    ...
    snarl_manager.for_each_snarl_preorder([&](const Snarl *snarl) {
      ...
      string emoji =
          ascii_labels ? picts.hashed_char(repr) : picts.hashed(repr);
  ...
  for (int i = 0; i < graph.node_size(); ++i) {
    ...
    if (ultrabubble_labeling) {
      ...
      if (noseq_mode) {
        inner_label << "<FONT COLOR=\"black\">" << n->id() << ":"
                    << n->sequence().size() << " bp</FONT> ";
      } else {
        inner_label << "<FONT COLOR=\"black\">" << n->id() << ":"
                    << n->sequence() << "</FONT> ";
      }
      for (auto &string_and_color : symbols_for_node[n->id()]) {
        ...
        inner_label << "<FONT COLOR=\"" << string_and_color.first << "\">"
                    << string_and_color.second << "</FONT>";
      }
      inner_label << "</TD>";
    } else if (simple_mode) {
      // inner_label << "<TD ROWSPAN=\"3\" BORDER=\"2\" CELLPADDING=\"5\">";
    ...
    if (simple_mode) {
      ...
      nlabel
          << "<TABLE BORDER=\"0\" CELLPADDING=\"0\" CELLSPACING=\"0\"><TR><TD "
             "PORT=\"nw\"></TD><TD PORT=\"n\"></TD><TD "
             "PORT=\"ne\"></TD></TR><TR><TD></TD><TD></TD></TR><TR><TD></TD>";
      nlabel << inner_label.str();
      nlabel << "<TD></TD></TR><TR><TD></TD><TD></TD></TR><TR><TD "
                "PORT=\"sw\"></TD><TD PORT=\"s\"></TD><TD "
                "PORT=\"se\"></TD></TR></TABLE>";
      nlabel << ">";
    }

    if (simple_mode) {
      out << "    " << n->id() << " [label=\"" << nlabel.str()
          << "\",penwidth=2,shape=circle,";
  ...
  if (annotate_paths) {
    // We're going to annotate the paths, so we need to give them symbols and
    // colors.
    Pictographs picts(random_seed);
    Colors colors(random_seed);
    // Work out what path symbols belong on what edges
    function<void(const Path &)> lambda = [this, &picts, &colors,
                                           &symbols_for_edge,
                                           &ascii_labels](const Path &path) {
      // Make up the path's label
      string path_label = ascii_labels ? picts.hashed_char(path.name())
                                       : picts.hashed(path.name());
      ...
      for (int i = 0; i < path.mapping_size(); ++i) {
        ...
        if (i < path.mapping_size() - 1) {
          ...
          if (!adjacent_mappings(m1, m2))
            continue;
          // Find the Edge connecting the mappings in the order they occur in
          // the path.
          Edge *edge_used =
              get_edge(NodeTraversal(get_node(m1.position().node_id()),
  ...
  for (int i = 0; i < graph.edge_size(); ++i) {
    Edge *e = graph.mutable_edge(i);
    max_edge_id =
        max((nid_t)max_edge_id, max((nid_t)e->from(), (nid_t)e->to()));
    ...
    // display what kind of edge we have using different edge head and tail
    // styles depending on if the edge comes from the start or not
    if (!simple_mode) {
      out << "    " << e->from() << " -> " << e->to();
      out << " [dir=both,";
      ...
      if (annotations != symbols_for_edge.end()) {
        ...
        for (auto &string_and_color : (*annotations).second) {
          // Put every symbol in its font tag.
          out << "<FONT COLOR=\"" << string_and_color.second << "\">"
              << string_and_color.first << "</FONT>";
        }
  ...
  for (auto &aln : alignments) {
    // check direction
    if (!aln.has_path())
      continue; // skip pathless alignments
    alnid++;
    ...
    for (int i = 0; i < aln.path().mapping_size(); ++i) {
      ...
      // void mapping_cigar(const Mapping& mapping, vector<pair<int, char> >&
      // cigar); string cigar_string(vector<pair<int, char> >& cigar); mapid <<
      // alnid << ":" << m.position().node_id() << ":" << cigar_string(cigar);
      ...
      if (i == 0) {
        out << "    " << alnid++ << " [label=\"" << aln.name()
            << setprecision(5) << "\n(" << aln.score() << " "
            << aln.mapping_quality() << " " << aln.identity() << ")"
            << "\",fontcolor=\"black\",fontsize=10];" << endl;
        out << "    " << alnid - 1 << " -> " << alnid
            << "[dir=none,color=\"gray\",style=\"dashed\",constraint=false];"
            << endl;
        out << "    " << alnid - 1 << " -> " << m.position().node_id()
            << "[dir=none,style=invis];" << endl;
        out << "    { rank = same; " << alnid - 1 << "; "
            << m.position().node_id() << "; };" << endl;
      }
      if (simple_mode) {
        out << "    " << alnid << " [label=\"" << m.position().node_id() << "\""
            << "shape=circle," // penwidth=2,"
            << "style=filled,"
            << "fillcolor=\"" << color << "\","
            << "color=\"" << color << "\"];" << endl;

      } else {
        out << "    " << alnid << " [label=\"" << mstr
            << "\",fontcolor=" << color << ",fontsize=10];" << endl;
      }
      if (i > 0) {
        out << "    " << alnid - 1 << " -> " << alnid
            << "[dir=none,color=\"black\",constraint=false];" << endl;
      }
      out << "    " << alnid << " -> " << m.position().node_id()
          << "[dir=none,style=invis];" << endl;
      out << "    { rank = same; " << alnid << "; " << m.position().node_id()
          << "; };" << endl;
      // out << "    " << m.position().node_id() << " -- " << alnid << "[color="
      // << color << ", style=invis];" << endl;
      alnid++;
    }
    alnid++;
  ...
  int locusid = alnid;
  {
    ...
    for (auto &locus : loci) {
      ...
      string path_label = ascii_labels ? picts.hashed_char(locus.name())
                                       : picts.hashed(locus.name());
      ...
      for (int j = 0; j < locus.allele_size(); ++j) {
        ...
        for (int i = 0; i < path.mapping_size(); ++i) {
          ...
          out << "    " << locusid << " [label=\"" << mapid.str()
              << "\",fontcolor=\"" << color << "\",fontsize=10];" << endl;
          if (i > 0) {
            out << "    " << locusid - 1 << " -> " << locusid
                << " [dir=none,color=\"" << color << "\",constraint=false];"
                << endl;
          }
          out << "    " << locusid << " -> " << m.position().node_id()
              << " [dir=none,style=invis];" << endl;
          out << "    { rank = same; " << locusid << "; "
              << m.position().node_id() << "; };" << endl;
          locusid++;
        }
      }
  ...
  if (show_paths || walk_paths) {
    ...
    function<void(const Path &)> lambda = [this, &pathid, &out, &picts, &colors,
                                           show_paths, walk_paths,
                                           show_mappings, &path_starts,
                                           &ascii_labels](const Path &path) {
      string path_label = ascii_labels ? picts.hashed_char(path.name())
                                       : picts.hashed(path.name());
      string color = colors.hashed(path.name());
        ...
        for (int i = 0; i < path.mapping_size(); ++i) {
          ...
          if (i == 0) { // add the path name at the start
            out << "    " << pathid << " [label=\"" << path_label << " "
                << path.name() << "  " << m.position().node_id() << " " << mstr
          ...
          out << "    " << pathid << " -> " << m.position().node_id()
              ...
              << "\", style=invis,constraint=false];" << endl;
          out << "    { rank = same; " << pathid << "; "
              << m.position().node_id() << "; };" << endl;
          pathid++;
          // if we're at the end
        ...
        for (int i = 0; i < path.mapping_size(); ++i) {
          ...
          if (i < path.mapping_size() - 1) {
            ...
            out << m1.position().node_id() << " -> " << m2.position().node_id()
                << " [dir=none,tailport="
                << (m1.position().is_reverse() ? "nw" : "ne")
                << ",headport=" << (m2.position().is_reverse() ? "ne" : "nw")
                << ",color=\"" << color << "\",label=\"     " << path_label
                << "     \",fontcolor=\"" << color << "\",constraint=false];"
                << endl;
          }
        }
        if (path.is_circular()) {
          ...
          const Mapping &m2 = path.mapping(0);
          out << m1.position().node_id() << " -> " << m2.position().node_id()
              << " [dir=none,tailport=ne,headport=nw,color=\"" << color
              << "\",label=\"     " << path_label << "     \",fontcolor=\""
              << color << "\",constraint=false];" << endl;
        }
      }
    };
    paths.for_each(lambda);
...
void VG::to_turtle(ostream &out, const string &rdf_base_uri, bool precompress) {
  ...
  for (int i = 0; i < graph.node_size(); ++i) {
    ...
    if (precompress) {
      ...
      out << "node:" << n->id() << " rdf:value \"" << n->sequence() << "\" . "
          << endl;
    }
  }
  function<void(const string &)> url_encode = [&out](const string &value) {
    out.fill('0');
  ...
  function<void(const Path &)> lambda = [&out, &precompress,
    ...
    for (auto &m : path.mapping()) {
      ...
      if (precompress) {
        ...
        out << " " << orientation << " node:" << m.position().node_id() << " ; "
            << endl;
        out << " vg:path path:";
        url_encode(path.name());
        out << " . " << endl;
...
void VG::connect_node_to_nodes(NodeTraversal node,
                               vector<NodeTraversal> &nodes) {
  for (vector<NodeTraversal>::iterator n = nodes.begin(); n != nodes.end();
       ++n) {
...
void VG::connect_nodes_to_node(vector<NodeTraversal> &nodes,
                               NodeTraversal node) {
  for (vector<NodeTraversal>::iterator n = nodes.begin(); n != nodes.end();
       ++n) {
    // Connect them left to right
...
void VG::add_start_end_markers(int length, char start_char, char end_char,
  ...
  for_each_node([&](Node *node) { unattached.insert(node); });

  // We handle the head and tail joining ourselves so we can do connected
  // components. We collect these before we add the new head/tail node so we
  // don't have to filter it out later.
  vector<Node *> heads;
  head_nodes(heads);
  vector<Node *> tails;
  tail_nodes(tails);
  ...
  if (start_node == nullptr) {
    // We get to create the node. In its forward orientation it's the start
    // node, so we use the start character.
  ...
  if (end_node == nullptr) {
    // We get to create the node. In its forward orientation it's the end node,
    // so we use the end character.
...
#ifdef debug
  cerr << "Start node is " << start_node->id() << ", end node is "
       << end_node->id() << endl;
#endif
  ...
  for (Node *head : heads) {
    if (unattached.count(head)) {
      ...
      for_each_connected_node(head,
                              [&](Node *node) { unattached.erase(node); });
    }
  ...
  while (!unattached.empty()) {
    ...
    // Mark everything it's attached to as attached
    for_each_connected_node(to_attach,
                            [&](Node *node) { unattached.erase(node); });

    // Add the edge
    create_edge(start_node, to_attach);
#ifdef debug
    cerr << "Added cycle-breaking edge " << start_node->id() << "->"
         << to_attach->id() << endl;
#endif
    vector<Edge *> edges;
    edges_of_node(to_attach, edges);
    for (auto edge : edges) {
      // cerr << "edge of " << to_attach->id() << " " << edge->from() << " " <<
      // edge->to() << endl;
      if (edge->to() == to_attach->id() && edge->from() != start_node->id()) {
        // cerr << "creating edge" << endl;
        Edge *created = create_edge(edge->from(), end_node->id(),
                                    edge->from_start(), false);
#ifdef debug
        cerr << "Added edge " << pb2json(*created) << " in response to "
             << pb2json(*edge) << endl;
#endif
      }
    }
...
unordered_map<nid_t, pair<nid_t, bool>> VG::overlay_node_translations(
    const unordered_map<nid_t, pair<nid_t, bool>> &over,
  ...
  for (auto &o : over) {
    ...
    if (u != under.end()) {
      ...
      overlay[new_id] = make_pair(oldest_id, is_rev ^ was_rev);
      /*
      cerr << "double trans "
           << new_id << " -> " << old_id
           << " -> " << oldest_id << endl;
...
Alignment VG::align(const Alignment &alignment, const Aligner *aligner,
                    const QualAdjAligner *qual_adj_aligner,
                    const vector<MaximalExactMatch> &mems, bool traceback,
                    bool acyclic_and_sorted, size_t max_query_graph_ratio,
                    bool pinned_alignment, bool pin_left, bool banded_global,
                    size_t band_padding_override, size_t max_span,
                    size_t unroll_length, int xdrop_alignment) {
...
#ifdef debug
  cerr << "aligning read of " << alignment.sequence().size() << " to graph of "
       << length() << endl;
  // cerr << pinned_alignment << " " << pin_left << " " << " " << banded_global
  // << " " << band_padding_override << " "  << max_span << endl;
#endif
  ...
  auto do_align = [&](VG &g) {
    ...
    if (aligner && qual_adj_aligner) {
      cerr << "error:[VG] cannot both adjust and not adjust alignment for base "
              "quality"
           << endl;
      exit(1);
    }
    if (banded_global) {
      ...
      // What band padding do we want? We used to hardcode it as 1, so it should
      // always be at least 1.
      size_t band_padding =
          permissive_banding ? max(max_span, (size_t)1) : band_padding_override;
#ifdef debug
      ...
      if (aligner && !qual_adj_aligner) {
        ...
        qual_adj_aligner->align_global_banded(aln, g, band_padding,
                                              permissive_banding);
      }
      ...
      if (aligner && !qual_adj_aligner) {
        aligner->align_xdrop(aln, g,
                             (translated_mems.size() ? translated_mems : mems),
                             xdrop_alignment != 1);
      } else {
  ...
  if (acyclic_and_sorted) {
    ...
    // with those scores?
    unroll_length =
        (unroll_length == 0 ? aln.sequence().size() : unroll_length);
    size_t component_length_max = 100 * unroll_length; // hard coded to be 100x

    // dagify the graph by unfolding inversions and then applying dagify forward
    // unroll
    VG dag = unfold(unroll_length, unfold_trans)
                 .dagify(unroll_length, dagify_trans, unroll_length,
    ...
    auto check_aln = [&](VG &graph, const Alignment &a) {
      if (a.has_path()) {
        ...
        // if (aln.sequence().find('N') == string::npos && seq !=
        // aln.sequence()) {
        if (seq != a.sequence()) {
    ...
    translate_nodes(aln, trans, [&](nid_t node_id) {
      // We need to feed in the lengths of nodes, so the offsets in the
      // alignment can be updated.
...
Alignment VG::align(const Alignment &alignment, const Aligner *aligner,
                    ...
                    size_t band_padding_override, size_t max_span,
                    size_t unroll_length, int xdrop_alignment) {
  return align(alignment, aligner, nullptr, mems, traceback, acyclic_and_sorted,
               max_query_graph_ratio, pinned_alignment, pin_left, banded_global,
               band_padding_override, max_span, unroll_length, xdrop_alignment);
}

Alignment VG::align(const Alignment &alignment, const Aligner *aligner,
                    bool traceback, bool acyclic_and_sorted,
                    size_t max_query_graph_ratio, bool pinned_alignment,
                    bool pin_left, bool banded_global,
                    size_t band_padding_override, size_t max_span,
                    size_t unroll_length, int xdrop_alignment) {
...
Alignment VG::align(const string &sequence, const Aligner *aligner,
                    bool traceback, bool acyclic_and_sorted,
                    size_t max_query_graph_ratio, bool pinned_alignment,
                    bool pin_left, bool banded_global,
                    size_t band_padding_override, size_t max_span,
  ...
  return align(alignment, aligner, traceback, acyclic_and_sorted,
               max_query_graph_ratio, pinned_alignment, pin_left, banded_global,
               band_padding_override, max_span, unroll_length, xdrop_alignment);
}

Alignment VG::align(const Alignment &alignment, bool traceback,
                    bool acyclic_and_sorted, size_t max_query_graph_ratio,
                    bool pinned_alignment, bool pin_left, bool banded_global,
                    size_t band_padding_override, size_t max_span,
                    size_t unroll_length, int xdrop_alignment) {
  Aligner default_aligner = Aligner();
  return align(alignment, &default_aligner, traceback, acyclic_and_sorted,
               max_query_graph_ratio, pinned_alignment, pin_left, banded_global,
               band_padding_override, max_span, unroll_length, xdrop_alignment);
}

Alignment VG::align(const string &sequence, bool traceback,
                    bool acyclic_and_sorted, size_t max_query_graph_ratio,
                    bool pinned_alignment, bool pin_left, bool banded_global,
                    size_t band_padding_override, size_t max_span,
                    size_t unroll_length, int xdrop_alignment) {
...
Alignment VG::align_qual_adjusted(const Alignment &alignment,
                                  ...
                                  bool pinned_alignment, bool pin_left,
                                  bool banded_global,
                                  size_t band_padding_override, size_t max_span,
                                  size_t unroll_length, int xdrop_alignment) {
  return align(alignment, nullptr, qual_adj_aligner, mems, traceback,
               acyclic_and_sorted, max_query_graph_ratio, pinned_alignment,
               pin_left, banded_global, band_padding_override, max_span,
               unroll_length, xdrop_alignment);
}
...
Alignment VG::align_qual_adjusted(const string &sequence,
  ...
  Alignment alignment;
  alignment.set_sequence(sequence);
  return align_qual_adjusted(
      alignment, qual_adj_aligner, traceback, acyclic_and_sorted,
      max_query_graph_ratio, pinned_alignment, pin_left, banded_global,
      band_padding_override, max_span, unroll_length, xdrop_alignment);
}
...
int VG::path_edge_count(list<NodeTraversal> &path, int32_t offset,
                        int path_length) {
  int edge_count = 0;
...
int VG::path_end_node_offset(list<NodeTraversal> &path, int32_t offset,
                             int path_length) {
  // This is the remaining path length
  int l = path_length;
...
void VG::collect_subgraph(Node *start_node, set<Node *> &subgraph) {
  ...
  while (!to_check.empty()) {
    ...
    for (auto *node : curr_check) {
      ...
      for (vector<NodeTraversal>::iterator p = prev.begin(); p != prev.end();
           ++p) {
      ...
      for (vector<NodeTraversal>::iterator n = next.begin(); n != next.end();
           ++n) {
  ...
  // cerr << "node " << start_node->id() << " subgraph size " << subgraph.size()
  // << endl;
}

bool VG::is_head_node(nid_t id) { return is_head_node(get_node(id)); }

bool VG::is_head_node(Node *node) { return start_degree(node) == 0; }
...
bool VG::is_tail_node(nid_t id) { return is_tail_node(get_node(id)); }

bool VG::is_tail_node(Node *node) { return end_degree(node) == 0; }
...
VG VG::unfold(uint32_t max_length,
...
#ifdef debug
    cerr << "[unfold] seeding orientation node " << inducing_node->id()
         << " fwd" << endl;
#endif
    ...
    while (!stack.empty()) {
      ...
      for (NodeTraversal next : travs_from(trav)) {
        if (main_orientation.count(next.node->id())) {
          ...
          if (next.backward != oriented_next.second) {
            // we've encountered this node in the opposite direction, so the
            // edge must have reversed the traversal relative to the induce
            // orientation
            reversing_edges.emplace(trav, next);
#ifdef debug
            cerr << "[unfold] new reversing edge " << trav << " -> " << next
                 << endl;
#endif
          } else if (!forward_edges.count(trav_edge)) {
            // we've never traversed this edge to this node, so add the edge and
            // move on
...
#endif
          // we've never seen this node before, so we induce the current
          // orientation on it
          Node *new_node = unfolded.create_node(
              next.backward ? reverse_complement(next.node->sequence())
                            : next.node->sequence());
          main_orientation[next.node->id()] =
              make_pair(new_node->id(), next.backward);
      ...
      for (NodeTraversal prev : travs_to(trav)) {
        if (main_orientation.count(prev.node->id())) {
...
#ifdef debug
            cerr << "[unfold] new reversing edge " << trav.reverse() << " -> "
                 << prev.reverse() << endl;
#endif
          } else if (!forward_edges.count(trav_edge)) {
            // we've never traversed this edge to this node, so add the edge and
          ...
          // orientation on it
          Node *new_node = unfolded.create_node(
              prev.backward ? reverse_complement(prev.node->sequence())
                            : prev.node->sequence());

          main_orientation[prev.node->id()] =
              make_pair(new_node->id(), prev.backward);
  ...
  // as an edge case, skip traversing the reverse strand if the search length is
  // 0
  if (!max_length) {
    return unfolded;
  }
  ...
  struct DistTraversal {
    ...
    inline bool operator<(const DistTraversal &other) const {
      return dist >
             other.dist; // opposite order so priority queue selects minimum
    }
  };

  // the IDs of nodes that we have duplicated in the reverse of the main
  // orientation
  ...
  // initialize the queue with the traversals along all of the reversing edges
  // we found during the orientation-inducing DFS
  ...
  for (auto search_init : reversing_edges) {
...
#ifdef debug
    cerr << "[unfold] intializing reverse strand search queue with "
         << init_trav << " -> " << init_next << " over edge "
         << pb2json(*init_edge) << endl;
#endif

    if (!reversed_nodes.count(init_next.node->id())) {
#ifdef debug
      cerr << "[unfold] reversed node " << init_next.node->id()
           << " has not been seen, adding to unfold graph" << endl;
#endif
      // we have not added this reverse node yet, so do it
      Node *rev_init_node = unfolded.create_node(
          main_orientation[init_next.node->id()].second
              ? init_next.node->sequence()
    ...
    if (init_trav.backward == main_orientation[init_trav.node->id()].second) {
      unfolded.create_edge(main_orientation[init_trav.node->id()].first,
                           reversed_nodes[init_next.node->id()]);
    } else {
      unfolded.create_edge(reversed_nodes[init_next.node->id()],
                           main_orientation[init_trav.node->id()].first);
    }
#ifdef debug
...
#ifdef debug
      cerr << "[unfold] traversal " << init_next
           << " has not been queued, adding to search queue" << endl;
#endif
      // we have not starting a traversal along this node in this direction, so
      // add it to the queue
...
#ifdef debug
    cerr << "[unfold] popping traversal " << dist_trav.trav << " at distance "
         << dist_trav.dist << endl;
#endif
...
#ifdef debug
      cerr << "[unfold] distance is above max of " << max_length
           << ", not continuing traversal" << endl;
#endif
      continue;
    }
...
#ifdef debug
      cerr << "[unfold] following edge " << pb2json(*edge) << " to " << next
           << endl;
#endif

      if ((next.backward == main_orientation[next.node->id()].second) ==
          (dist_trav.trav.backward ==
           main_orientation[dist_trav.trav.node->id()].second)) {
#ifdef debug
        cerr << "[unfold] stays on reverse strand" << endl;
#endif
        if (!reversed_nodes.count(next.node->id())) {
          // this is the first time we've encountered this node in any direction
          // on the reverse strand so add the reverse node
          Node *rev_node = unfolded.create_node(
              main_orientation[next.node->id()].second
                  ? next.node->sequence()
                  : reverse_complement(next.node->sequence()));
          reversed_nodes[next.node->id()] = rev_node->id();

#ifdef debug
          cerr << "[unfold] node has not been observed on reverse strand, "
                  "adding reverse node "
               << rev_node->id() << endl;
#endif
        }
        ...
        if (!reversed_edges.count(edge)) {
          if (dist_trav.trav.backward ==
              main_orientation[dist_trav.trav.node->id()].second) {
            unfolded.create_edge(reversed_nodes[next.node->id()],
                                 reversed_nodes[dist_trav.trav.node->id()]);
          } else {
            unfolded.create_edge(reversed_nodes[dist_trav.trav.node->id()],
                                 reversed_nodes[next.node->id()]);
          }
          reversed_edges.insert(edge);
#ifdef debug
          cerr << "[unfold] edge has not been observed on reverse strand, "
                  "adding reverse edge"
               << endl;
#endif
        }
        ...
        if (!queued.count(make_pair(next.node->id(), next.backward))) {
          // this is the first time we've encountered this node in this
          // direction on the reverse strand so queue it up for the search
          // through
#ifdef debug
          cerr << "[unfold] traversal " << next
               << " has not been seen on reverse strand, queueing up" << endl;
#endif
  ...
  for (auto orient_record : main_orientation) {
    node_translation[orient_record.second.first] =
        make_pair(orient_record.first, orient_record.second.second);
  }
  for (auto rev_orient_record : reversed_nodes) {
    node_translation[rev_orient_record.second] =
        make_pair(rev_orient_record.first,
...
VG VG::dagify(uint32_t expand_scc_steps,
              unordered_map<nid_t, pair<nid_t, bool>> &node_translation,
              size_t target_min_walk_length, size_t component_length_max) {
  ...
  // that maps the unrolled id to the original node and whether we've inverted
  // or not
  ...
  for (auto &id : weak_components) {
    ...
    for (auto e : edges_of(get_node(id))) {
      if (weak_components.count(e->from()) && weak_components.count(e->to())) {
        dag.add_edge(*e);
      }
    }
  }
  ...
  for (auto &component : strongly_connected_and_self_looping_components) {
    ...
    // copy the SCC expand_scc_steps times, each time forwarding links from the
    // old copy into the new the result is a DAG even if the graph is initially
    // cyclic
    ...
    for (uint32_t i = 0; i < expand_scc_steps + 1; ++i) {
      ...
      // And connect to the nodes in this and the previous component using the
      // original edges as guide We will break any cycles this introduces at
      // each step
      set<nid_t> seen;
      for (auto id : component) {
        seen.insert(id);
        for (Edge *e : edges_of(get_node(id))) {
          ...
          if (e->from() == id && e->to() != id) {
            ...
            if (!last.empty() && component.count(e->from())) {
              ...
              // add the edge that connects back to the previous node in the
              // last copy
              Edge new_edge = *e;
              new_edge.set_to(curr[id]->id());
              ...
              size_t inmm = curr[id]->sequence().size() +
                            min_return_length[last[e->from()]];
              mm = (mm ? min(mm, inmm) : inmm);
              curr_min_min_return_length =
                  (curr_min_min_return_length
                       ? min(mm, curr_min_min_return_length)
                       : mm);
            }
          } else if (e->to() == id && e->from() == id) {
            // we don't add the self loop because we would just need to remove
            // it anyway
            if (!last.empty()) { // by definition, we are looking at nodes in
                                 // this component
      ...
      if (component_length_max && component_length >= component_length_max)
        break;
    }
  }

  // ensure normalized edges in output; we may have preserved some
  // flipped/flipped edges which are valid but can introduce problems for some
  // algorithms

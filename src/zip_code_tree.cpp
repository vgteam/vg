...
void ZipCodeForest::open_chain(forest_growing_state_t& forest_state, 
    ...
    if (chain_is_reversed) {
        ...
        if (chain_length == std::numeric_limits<size_t>::max()) {
            forest_state.sibling_indices_at_depth[depth-1].back().distances.first = std::numeric_limits<size_t>::max();
        } else {
            size_t dist_val = forest_state.sort_values_by_seed[seed_index].get_distance_value();
            bool child_is_node = is_node || current_seed.zipcode.get_code_type(depth+1) == ZipCode::NODE;
            // Offset is distance plus possibly the child's length
            size_t offset = SnarlDistanceIndex::sum(dist_val, child_is_node ? 0
            ...
            forest_state.sibling_indices_at_depth[depth-1].back().distances.first 
                = SnarlDistanceIndex::minus(chain_length, offset);
        }
    } else {
...
void ZipCodeForest::add_child_to_chain(forest_growing_state_t& forest_state, const size_t& depth,
    ...
    if (chain_depth > 0 && first_type == ZipCodeTree::CHAIN_START) {
        ...
        size_t own_length = (is_trivial_chain || current_type == ZipCode::NODE) 
                            ? 0 
                            : current_seed.zipcode.get_length(chain_depth+1);
        forest_state.sibling_indices_at_depth[chain_depth-1][0].distances.first = chain_is_reversed 
            ? SnarlDistanceIndex::minus(current_seed.zipcode.get_length(chain_depth, true),
                SnarlDistanceIndex::sum(current_offset, own_length))
            : current_offset;
...
void ZipCodeForest::close_snarl(forest_growing_state_t& forest_state,
    ...
    if (trees[forest_state.active_tree_index].zip_code_tree.size() == 1) {
        ...
        trees[forest_state.active_tree_index].zip_code_tree.emplace_back(
            ZipCodeTree::DAG_SNARL_END, snarl_start.snarl_id);
    ...
    assert(trees[forest_state.active_tree_index].zip_code_tree.back().get_type() == ZipCodeTree::CHAIN_START);
        ...
        trees[forest_state.active_tree_index].zip_code_tree.emplace_back(closing_type, snarl_start.snarl_id);
     }
}
...
ZipCodeForest::seed_info_t::seed_info_t(size_t index, bool is_rev, size_t flank, bool right_side,
                                        ...
                                        zipcode(forest_state.seeds->at(index).zipcode),
                                        flank_offset(flank), right_side(right_side) {
...
vector<ZipCodeForest::seed_info_t> ZipCodeForest::get_edge_seeds(const forest_growing_state_t& forest_state, 
    ...
    for (size_t i = 1; i < chain_indices.size(); ++i) {
        ...
        while (active_zip_tree[seed_i].get_type() != ZipCodeTree::SEED) {
            seed_i++;
        }
        edge_seeds.emplace_back(active_zip_tree[seed_i].get_value(), !active_zip_tree[seed_i].get_is_reversed(),
            cur_chain.distances.first, cur_chain.is_reversed, depth, forest_state);

        // Search for last seed, starting from right before chain end
        seed_i = i == chain_indices.size() - 1 ? active_zip_tree.size() - 1
                                               : chain_indices[i+1].value - 1;
        while (active_zip_tree[seed_i].get_type() != ZipCodeTree::SEED) {
            seed_i--;
        }
        edge_seeds.emplace_back(active_zip_tree[seed_i].get_value(), active_zip_tree[seed_i].get_is_reversed(),
            cur_chain.distances.second, !cur_chain.is_reversed, depth, forest_state);
    }
...
ZipCodeTree::seed_iterator::seed_iterator(vector<tree_item_t>::const_iterator begin, 
    const ZipCodeTree& ziptree) : it(begin), end(ziptree.zip_code_tree.end()), zip_code_tree(ziptree.zip_code_tree),
    snarl_start_indexes(ziptree.snarl_start_indexes), cyclic_snarl_nestedness(0), chain_numbers(std::stack<size_t>()) {
...
ZipCodeTree::distance_iterator::distance_iterator(vector<tree_item_t>::const_reverse_iterator rbegin, 
    const vector<tree_item_t>& zip_code_tree, const unordered_map<size_t, size_t>& snarl_start_indexes,
    std::stack<size_t> chain_numbers, bool right_to_left, size_t distance_limit) :
    it(rbegin), origin(rbegin), rend(right_to_left ? zip_code_tree.rend() : zip_code_tree.rbegin()),

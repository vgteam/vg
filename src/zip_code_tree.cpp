...
void ZipCodeForest::add_child_to_chain(forest_growing_state_t& forest_state, const size_t& depth,
    ...
    // so it is stored in the forest_state instead
    child_info_t& cur_chain = chain_depth == 0 ? forest_state.active_top_level_chain
                                               : forest_state.sibling_indices_at_depth[chain_depth-1].back();
    ...
    if (chain_depth > 0 && first_type == ZipCodeTree::CHAIN_START) {
        ...
        const auto& flank_distances = cur_chain.distances.first;
        ...
        // Update distances since last loop
        cur_chain.dist_since_forward_loop = forest_state.distance_index->sum(cur_chain.dist_since_forward_loop, 
                                                                             distance_between);
        cur_chain.dist_since_reverse_loop = forest_state.distance_index->sum(cur_chain.dist_since_reverse_loop, 
                                                                             distance_between);
...
#ifdef DEBUG_ZIP_CODE_TREE
                    assert(cur_chain.type == ZipCodeTree::CHAIN_START);   
                    // The value should be the index of the last seed,
                    // which is the first seed in the new tree
                    assert(cur_chain.value == trees[forest_state.active_tree_index].zip_code_tree.size()-1);
    ...
    if ((rev_relative_to_index && current_seed.zipcode.get_has_forward_loop(depth))
        ...
        if (cur_chain.has_reverse_loop()) {
            ...
            const tree_item_t& item_after_loop = \
                trees[forest_state.active_tree_index].zip_code_tree[cur_chain.last_reverse_loop_index + 1];

            if (item_after_loop.get_type() == ZipCodeTree::SEED 
                && id(forest_state.seeds->at(item_after_loop.get_value()).pos) == id(current_seed.pos)) {
                // Loops for the same node ID are by default merge-able
#ifdef DEBUG_ZIP_CODE_TREE
                cerr << "\tNope, this node ID already has a reverse loop" << endl;
#endif
                add_loop = false;
            }
        }
    ...
    if ((rev_relative_to_index && current_seed.zipcode.get_has_reverse_loop(depth))
        ...
        if (cur_chain.has_forward_loop() && !remove_prior) {
            ...
            if (dist_using_new_loop == prev_loop_item.get_value()) {
#ifdef DEBUG_ZIP_CODE_TREE
...
size_t ZipCodeForest::get_loop_distance(const SnarlDistanceIndex* distance_index, const Seed& seed,
    ...
    if (seed.zipcode.get_code_type(depth) == ZipCode::NODE) {
        ...
        loop_anchor = distance_index->get_node_net_handle(id(seed.pos));
    } else {
...
void ZipCodeForest::close_snarl(forest_growing_state_t& forest_state,
    ...
    if (active_zip_tree.size() == 1) {
        ...
        // We might've stored a forward loop right inside the snarl
        bool move_forward_loop = active_zip_tree[snarl_start_i+1].get_type() == ZipCodeTree::LOOP;
        ...
        for (size_t i = 1; i < chain_indices.size(); ++i) {
            ...
            active_zip_tree[chain_start_i].set_value(chain_start_i - snarl_start_i - move_forward_loop);
            active_zip_tree[chain_end_i].set_value(chain_end_i - snarl_start_i - move_forward_loop);
            // Also set is_cyclic
            active_zip_tree[chain_start_i].set_is_cyclic(is_cyclic);
            active_zip_tree[chain_end_i].set_is_cyclic(is_cyclic);
        }

        // Take out the forward loop
        tree_item_t forward_loop;
        if (move_forward_loop) {
            forward_loop = active_zip_tree[snarl_start_i+1];
            active_zip_tree.erase(active_zip_tree.begin() + snarl_start_i+1);
        }
        ...
        // Move forward to outside the right end
        if (move_forward_loop) {
        ...
        // Reverse loop now has to go through the snarl
        if (active_chain.has_reverse_loop()) {
            active_chain.dist_since_reverse_loop += last_seed.zipcode.get_length(depth);
        }
    }
}
...
void ZipCodeForest::add_edges_to_end(vector<tree_item_t>& dist_matrix,
    ...
    if (is_cyclic_snarl) {
        // end -> end may be possible, but we handle with LOOPs already
...
size_t ZipCodeForest::add_distance_matrix(forest_growing_state_t& forest_state, 
    ...
    if (is_cyclic_snarl) {
        // start -> start may be possible, but we handle with LOOPs already
        dist_matrix.emplace_back(ZipCodeTree::EDGE, std::numeric_limits<size_t>::max());
    }
...
ZipCodeTree::seed_iterator::seed_iterator(size_t start_index, const ZipCodeTree& ziptree)
    : zip_code_tree(&ziptree.zip_code_tree), index(start_index), right_to_left(true),
...
auto ZipCodeTree::distance_iterator::operator*() const -> seed_result_t {
    ...
    crash_unless(!pos.stack_data.empty());
#endif
    ...
    size_t distance = pos.stack_data.top();
    bool is_reversed = pos.right_to_left ? (*current_item).get_is_reversed()
                                         : !(*current_item).get_is_reversed();
    return {distance, (*current_item).get_value(), is_reversed};
}

auto ZipCodeTree::distance_iterator::push(size_t value) -> void {
    pos.stack_data.push(value);
}

auto ZipCodeTree::distance_iterator::pop() -> size_t {
    size_t value = pos.stack_data.top();
    pos.stack_data.pop();
    return value;
}
...
size_t ZipCodeTree::distance_iterator::get_cyclic_snarl_bound_distance(size_t snarl_start_i, size_t row, bool to_end) {
...
#ifdef debug_parse
    std::cerr << "\tTo current running parent distance " << top() << " add edge " 
              << (edge_dist == std::numeric_limits<size_t>::max() ? "inf" : std::to_string(edge_dist))
              << " to get to snarl bound" << std::endl;
#endif
    ...
    if (old_dist == best_so_far_distance_cache.end()) {
        ...
        if (to_end) {
            exit_dists = {std::numeric_limits<size_t>::max(), total_dist};
        } else {
    ...
    } else if (!to_end && old_dist->second.first > total_dist) {
        // Update best distance to snarl start
        old_dist->second.first = total_dist;
    } else {
#ifdef debug_parse
...
void ZipCodeTree::distance_iterator::skip_chain() {
    ...
    change_index_to(pop());
#ifdef debug_parse
    std::cerr << "Skip chain, jump to index " << pos.index << std::endl;
#endif
...
void ZipCodeTree::distance_iterator::initialize_chain() {
    ...
    push(pos.index + (*current_item).get_other_bound_offset());
    swap();
...
void ZipCodeTree::distance_iterator::save_loop_traversal() {
#ifdef debug_parse
    std::cerr << "\tSave reversal use at index " << pos.index
              << " with current distance " << pos.stack_data.top() << std::endl;
#endif
...
void ZipCodeTree::distance_iterator::initialize_snarl(size_t chain_num) {
    ...
    std::cerr << "Initialize snarl from chain number " << chain_num 
              << " going " << (pos.right_to_left ? "right to left" : "left to right")
    ...
    size_t snarl_start_i = pos.index - (*current_item).get_value();
    bool is_cyclic = zip_code_tree->at(snarl_start_i).get_is_cyclic();
    bool start_right_to_left = pos.right_to_left;
    ...
    if (is_cyclic) {
        if (chain_num == 0 || chain_num == std::numeric_limits<size_t>::max()) {
            ...
            pop();
            // Jump to snarl end
            change_index_to(snarl_start_i);
            if (!pos.right_to_left) {
                shift_index_by(zip_code_tree->at(snarl_start_i).get_other_bound_offset());
            }
#ifdef debug_parse
            std::cerr << "Skip cyclic snarl, jump to index " << pos.index << std::endl;
#endif
    ...
    if (is_cyclic || chain_num == 0) {
        ...
        shift_index_by(is_cyclic ? (node_count*2 * (node_count*2 + 1)) / 2
                                 : (node_count * (node_count + 1)) / 2);
#ifdef debug_parse
        std::cerr << "Jump to index " << pos.index << " after distance matrix" << std::endl;
#endif
    }
}
...
void ZipCodeTree::distance_iterator::unimplemented_error() {
    throw std::domain_error("Unimplemented symbol " + std::to_string((*current_item).get_type()) 
                                    + " for state " + std::to_string(pos.state));
}

auto ZipCodeTree::distance_iterator::tick() -> bool {
#ifdef debug_parse
    std::cerr << "Tick for state " << pos.state << " on symbol " << (*current_item).get_type() 
              << " at entry " << pos.index << std::endl;
    std::cerr << "Stack: ";
    std::stack<size_t> temp_stack = pos.stack_data;
    ...
    case S_SCAN_CHAIN:
        ...
        if ((*current_item).get_type() == SEED) {
            ...
            bool cur_is_rev = pos.right_to_left ? (*current_item).get_is_reversed() 
                                                : !(*current_item).get_is_reversed();
...
#endif
                    size_t chain_num = pos.chain_numbers.top();
                    pos.chain_numbers.pop();
            ...
            if (old_dist == best_so_far_distance_cache.end()) {
                ...
                if ((pos.right_to_left && top() >= old_dist->second.first)
                    || (!pos.right_to_left && top() >= old_dist->second.second)) {
#ifdef debug_parse
                    ...
                    if (pos.right_to_left) {
                        old_dist->second.first = top();
                    } else {
                        old_dist->second.second = top();
                    }
                }
            }
        } else {
    ...
    case S_SCAN_DAG_SNARL:
        ...
        if ((*current_item).get_type() == EDGE) {
            ...
            shift_index_by(1);
            shift_index_by(-(*current_item).get_value());
    ...
    case S_SCAN_CYCLIC_SNARL:
        ...
        if ((*current_item).get_type() == SNARL_END) {
...
#ifdef check_parse
            crash_unless(!pos.right_to_left);
#endif
            pos.right_to_left = true;
        } else if ((*current_item).get_type() == EDGE) {
            ...
            if (!pos.right_to_left) {
                // Jump to snarl end
                shift_index_by((*current_item).get_other_bound_offset());
            }
#ifdef debug_parse
            std::cerr << "Jump to index " << pos.index << endl;
#endif
            pos.state = S_SCAN_CHAIN;
        } else if (entered_chain()) {
            // We've encountered a chain to look at, and the running distance
        ...
        break;
    case S_TAKE_LOOP:
        // State where we have just reloaded to a loop we are considering taking
        // We just check if we've ever seen this loop at a better distance,
        // and if not before, then we turn around.
        if ((*current_item).get_type() == LOOP) {
            // Loop around
            ...
            if (old_dist_val <= top()) {
#ifdef debug_parse
                cerr << "We've been here before with dist " << old_dist_val
                     << " < " << top() << endl;
#endif
                halt();
            } else {
...
auto ZipCodeTree::find_distances(const seed_iterator& from, size_t distance_limit) const -> distance_iterator {
    return distance_iterator(from, distance_limit);
}
...
void ZipCodeForest::sort_one_interval(forest_growing_state_t& forest_state, const interval_state_t& interval) const {
    ...
    for (size_t i = interval.interval_start ; i < interval.interval_end ; i++) {
        ...
        if (interval.code_type == ZipCode::EMPTY) {
            ...
            get_chain_child_sort_info(sort_values_by_seed[zipcode_sort_order[i]], seed, interval.depth+1);
            min_component = std::min(min_component, sort_values_by_seed[zipcode_sort_order[i]].get_chain_component());
            max_component = std::max(max_component, sort_values_by_seed[zipcode_sort_order[i]].get_chain_component());
#ifdef DEBUG_ZIP_CODE_SORTING
            cerr << "Prefix sum " << sort_values_by_seed[zipcode_sort_order[i]].get_distance_value()
                 ...
                 << " and type " << sort_values_by_seed[zipcode_sort_order[i]].get_code_type() << endl;
#endif
        } else {
#ifdef DEBUG_ZIP_CODE_SORTING
...
void ZipCodeForest::get_chain_child_sort_info(sort_value_t& chain_child, const Seed& seed, const size_t& depth) const {
    if (depth == 0) {
        throw std::runtime_error("Depth 0 is not a child. Do your zipcode, minimizer, and graph files match?");
    }

    if (seed.zipcode.decoder[depth].is_chain) {
        // is_chain so could be a chain or a node
        if (seed.zipcode.decoder[depth-1].is_chain) {
            // If the thing before this was also a chain, then it is a node
            chain_child.set_code_type(ZipCode::NODE);
            size_t zip_value;
            ...
            size_t offset_in_chain = zip_value == 0 ? std::numeric_limits<size_t>::max() : zip_value-1;
            // Next is the length
            std::tie(zip_value, zip_index) = seed.zipcode.zipcode.get_value_and_next_index(zip_index);
            size_t length = 0 ? std::numeric_limits<size_t>::max() : zip_value-1;
            // Next is the reversedness
            std::tie(zip_value, zip_index) = seed.zipcode.zipcode.get_value_and_next_index(zip_index);
            bool is_reversed = zip_value & ZipCode::NODE_FLAG_IS_REVERSED;
            ...
            size_t node_offset = is_reversed != is_rev(seed.pos) ? length - offset(seed.pos)
                                                                 : offset(seed.pos);
            chain_child.set_sort_value(SnarlDistanceIndex::sum(offset_in_chain, node_offset));
            chain_child.set_chain_order(node_offset == 0 ? 2 : 0);
        } else {
            throw std::runtime_error("A chain is not a child of a chain. Do your zipcode, minimizer, and graph files match?");
        }
    } else {
        // Definitely a snarl
        chain_child.set_chain_order(1);
        size_t zip_value;
        size_t zip_index = seed.zipcode.decoder[depth].offset;
        std::tie(zip_value, zip_index) = seed.zipcode.zipcode.get_value_and_next_index(zip_index);
        // First is the snarl type
        if (zip_value & ZipCode::SNARL_FLAG_IS_REGULAR) {
            chain_child.set_code_type(ZipCode::REGULAR_SNARL);
        } else if (zip_value & ZipCode::SNARL_FLAG_IS_CYCLIC) {
        ...
        // Next is the offset
        std::tie(zip_value, zip_index) = seed.zipcode.zipcode.get_value_and_next_index(zip_index);
        chain_child.set_sort_value(zip_value == 0 ? std::numeric_limits<size_t>::max() : zip_value-1);
        // Three from now is the chain component
        std::tie(zip_value, zip_index) = seed.zipcode.zipcode.get_value_and_next_index(zip_index);
        std::tie(zip_value, zip_index) = seed.zipcode.zipcode.get_value_and_next_index(zip_index);
        std::tie(zip_value, zip_index) = seed.zipcode.zipcode.get_value_and_next_index(zip_index);
        chain_child.set_chain_component(zip_value);
    ...
    while (!forest_state.intervals_to_process.empty()) {
        ...
        if (forest_state.open_intervals.empty()) {
            ...
            if (current_interval.code_type == ZipCode::ROOT_SNARL) {
                ...
                forest_state.sibling_indices_at_depth[0].emplace_back(ZipCodeTree::CHAIN_START, 0);
                forest_state.sibling_indices_at_depth[0].back().chain_component = 0;
                // Fresh top level chain loop storage
                forest_state.active_top_level_chain.reset_loop_storage();
            }                       
...
std::string to_string(const vg::ZipCodeTree::distance_iterator::State& state) {
    ...
    case vg::ZipCodeTree::distance_iterator::S_TAKE_LOOP:
        return "S_TAKE_LOOP";
    default:

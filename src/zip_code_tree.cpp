...
void ZipCodeForest::open_chain(forest_growing_state_t& forest_state, 
...
#endif
    bool parent_is_cyclic_snarl = forest_state.sibling_indices_at_depth[depth-1][0].type == ZipCodeTree::CYCLIC_SNARL_START;
    ...
    if (depth == 0) {
        ...
        if (!parent_is_cyclic_snarl) {
            // Cyclic snarls have all distances at the front
    ...
    auto opening_type = parent_is_cyclic_snarl ? ZipCodeTree::CYCLIC_SNARL_CHAIN_START : ZipCodeTree::CHAIN_START;
    auto snarl_id = parent_is_cyclic_snarl ? forest_state.sibling_indices_at_depth[depth-1][0].value : std::numeric_limits<size_t>::max();
    trees[forest_state.active_tree_index].zip_code_tree.emplace_back(opening_type, snarl_id, false);

    //Remember the start of the chain and its prefix sum value as a child of the chain
    forest_state.sibling_indices_at_depth[depth].push_back({opening_type, chain_is_reversed ? current_seed.zipcode.get_length(depth, true)
                                                                                                       : 0});
    ...
    if (depth != 0) {
        forest_state.sibling_indices_at_depth[depth-1].push_back({opening_type,
...
bool ZipCodeForest::move_slice(forest_growing_state_t& forest_state, const size_t& depth,
    ...
    auto start_of_slice = trees[forest_state.active_tree_index].zip_code_tree.begin() 
    ...
    bool move_full_chain = start_of_slice->get_type() == ZipCodeTree::CHAIN_START 
                            || start_of_slice->get_type() == ZipCodeTree::CYCLIC_SNARL_CHAIN_START;
    ...
    if (!has_end) {
        ...
        trees[forest_state.active_tree_index].zip_code_tree.emplace_back(
            ZipCodeTree::CHAIN_END, std::numeric_limits<size_t>::max(), false);
        end_of_slice++;
    }
    trees.emplace_back();
    ...
    if (move_full_chain) {
        ...
        if (forest_state.open_chains.back().first != 0) {
            ...
            if (parent_is_cyclic_snarl) {
                // The bounds should be normal chain bounds - not inside cyclic snarl any more
                trees.back().zip_code_tree.front() = 
                    ZipCodeTree::tree_item_t(ZipCodeTree::CHAIN_START, std::numeric_limits<size_t>::max(), false);
                trees.back().zip_code_tree.back() = 
                    ZipCodeTree::tree_item_t(ZipCodeTree::CHAIN_END, std::numeric_limits<size_t>::max(), false);
            }
        ...
        assert(start_of_slice->get_type() == ZipCodeTree::SEED || start_of_slice->get_type() == ZipCodeTree::SNARL_START
               || start_of_slice->get_type() == ZipCodeTree::CYCLIC_SNARL_START);
#endif
...
void ZipCodeForest::close_chain(forest_growing_state_t& forest_state, 
    ...
    auto closing_type = parent_is_cyclic_snarl ? ZipCodeTree::CYCLIC_SNARL_CHAIN_END : ZipCodeTree::CHAIN_END;
    auto snarl_id =parent_is_cyclic_snarl ? forest_state.sibling_indices_at_depth[depth-1][0].value 
                                          : std::numeric_limits<size_t>::max();
#ifdef DEBUG_ZIP_CODE_TREE
    cerr << "\t\tclose a chain at depth " << depth  << " in ";
    cerr << (parent_is_cyclic_snarl ? "cyclic snarl" : "non-cyclic snarl") << endl;
#endif
    if (trees[forest_state.active_tree_index].zip_code_tree.back().get_type() == ZipCodeTree::CHAIN_START
        ...
        trees[forest_state.active_tree_index].zip_code_tree.emplace_back(closing_type, snarl_id, false);
        ...
        assert((!parent_is_cyclic_snarl 
                && forest_state.sibling_indices_at_depth[depth-1].back().type == ZipCodeTree::CHAIN_START)
               || (parent_is_cyclic_snarl
                  && forest_state.sibling_indices_at_depth[depth-1].back().type == ZipCodeTree::CYCLIC_SNARL_CHAIN_START));
#endif
        ...
        if (distance_to_chain_end > forest_state.distance_limit && forest_state.open_chains.back().second) {
            ...
            bool moved_full_chain = move_slice(forest_state, depth, parent_is_cyclic_snarl, true);
        ...
        if (add_distances) {
            ...
            if (!is_cyclic_snarl) {
                // Cyclic snarls don't store distances between chains
                add_snarl_distances(forest_state, depth-1, last_seed, chain_is_reversed, snarl_is_reversed, false);
            }
        }
...
void ZipCodeForest::add_child_to_chain(forest_growing_state_t& forest_state, 
    ...
    bool parent_is_cyclic_snarl = depth > 0 && forest_state.sibling_indices_at_depth[depth-1].size() > 0
                                  && forest_state.sibling_indices_at_depth[depth-1][0].type == ZipCodeTree::CYCLIC_SNARL_START;
    bool grandparent_is_cyclic_snarl = depth > 1 && forest_state.sibling_indices_at_depth[depth-2].size() > 0
                                       && forest_state.sibling_indices_at_depth[depth-2][0].type == ZipCodeTree::CYCLIC_SNARL_START;
    auto opening_type = parent_is_cyclic_snarl ? ZipCodeTree::CYCLIC_SNARL_CHAIN_START : ZipCodeTree::CHAIN_START;
    auto closing_type = parent_is_cyclic_snarl ? ZipCodeTree::CYCLIC_SNARL_CHAIN_END : ZipCodeTree::CHAIN_END;
    auto snarl_id = parent_is_cyclic_snarl ? forest_state.sibling_indices_at_depth[depth-1][0].value : std::numeric_limits<size_t>::max();
    ...
    bool first_is_chain_start = (first_type == ZipCodeTree::CHAIN_START || first_type == ZipCodeTree::CYCLIC_SNARL_CHAIN_START);
    // Assume we want to use this child
    bool skip_child = false;
    if (chain_depth > 0 && first_is_chain_start) {
        ...
        if (chain_depth == 0 && distance_between > forest_state.distance_limit) {
            ...
            forest_state.sibling_indices_at_depth[chain_depth].push_back({opening_type, chain_is_reversed ? current_seed.zipcode.get_length(chain_depth, true)
                                                                                                       : 0}); 
            ...
            if (forest_state.open_chains.back().second) {
...
#endif
                bool moved_full_chain = move_slice(forest_state, chain_depth, parent_is_cyclic_snarl, false);
                ...
                if (moved_full_chain) {
                    ...
                    assert(forest_state.sibling_indices_at_depth[chain_depth-1].back().type == ZipCodeTree::CHAIN_START
                           || forest_state.sibling_indices_at_depth[chain_depth-1].back().type == ZipCodeTree::CYCLIC_SNARL_CHAIN_START);   
            ...
            auto prev_type = trees[forest_state.active_tree_index].zip_code_tree.back().get_type();
            if (distance_between == 0 && prev_type == ZipCodeTree::SEED && 
...
void ZipCodeForest::close_snarl(forest_growing_state_t& forest_state, 
    ...
    if (trees[forest_state.active_tree_index].zip_code_tree.size() == 1) {
        ...
        //Root snarls are always non-cyclic (right?)
        ...
        assert(trees[forest_state.active_tree_index].zip_code_tree.back().get_type() == ZipCodeTree::SNARL_START
                || trees[forest_state.active_tree_index].zip_code_tree.back().get_type() == ZipCodeTree::CYCLIC_SNARL_START);
#endif        
        ...
        auto closing_type = is_cyclic_snarl ? ZipCodeTree::CYCLIC_SNARL_END : ZipCodeTree::SNARL_END;
        ...
        if (sibling.type == ZipCodeTree::SNARL_START) {
            ...
            if (seed.zipcode.get_code_type(depth) == ZipCode::REGULAR_SNARL) {
                ...
                if (to_snarl_end) {
                    ...
                    size_t rank1 = sibling_seed.zipcode.get_rank_in_snarl(depth+1);
                    bool right_side1 = !sibling.is_reversed;

                    size_t distance_to_end_of_last_child = sibling.distances.second;
...
void ZipCodeForest::add_cyclic_snarl_distances(forest_growing_state_t& forest_state, 
...
#ifdef DEBUG_ZIP_CODE_TREE
    cerr << "\t\tAdd distances for cyclic snarl at depth " << depth << endl;
#endif
    ...
    struct seed_info_t {
        ...
        seed_info_t(size_t i, size_t f, bool s) : index(i), flank_offset(f), side(s) {}
    };

    // Seed indexes & their flank offsets
    vector<seed_info_t> seeds;
    // Get the first seeds of each chain
    for (size_t i = 1; i < sibling_count; ++i) {
        ...
        seeds.emplace_back(trees[forest_state.active_tree_index].zip_code_tree[seed_i].get_value(),
            cur_chain.distances.first, cur_chain.is_reversed);

        // Search for chain end
        while (trees[forest_state.active_tree_index].zip_code_tree[seed_i].get_type() != ZipCodeTree::CYCLIC_SNARL_CHAIN_END) {
            seed_i++;
        }
        ...
        seeds.emplace_back(trees[forest_state.active_tree_index].zip_code_tree[seed_i].get_value(),
            cur_chain.distances.second, !cur_chain.is_reversed);
    }

    net_handle_t snarl_handle = forest_state.seeds->at(seeds[0].index).zipcode.get_net_handle(
                                     depth, forest_state.distance_index);

    // Cyclic snarls store child count at the start (all siblings minus the snarl start)
    dist_matrix.emplace_back(ZipCodeTree::NODE_COUNT, sibling_count - 1, false);

    // start -> start
    dist_matrix.emplace_back(ZipCodeTree::EDGE, 
        forest_state.distance_index->distance_in_snarl(snarl_handle, 0, true, 0, true), false);

    // Most of the triangle
    for (size_t i = 0; i < seeds.size(); ++i) {
        // Current chain
        auto& first_seed = forest_state.seeds->at(seeds[i].index);
        size_t first_seed_rank = first_seed.zipcode.get_rank_in_snarl(depth+1);
        ...
        for (int64_t j = i; j >= 0; --j) {
            ...
            const auto& next_seed = forest_state.seeds->at(seeds[j].index);
            size_t next_rank = next_seed.zipcode.get_rank_in_snarl(depth+1);
            ...
            if (i == j) {
                ...
                if (between_chain_dist != std::numeric_limits<size_t>::max()) {
                    edge_dist = chain_flank_dist;
                } else {
                    edge_dist = std::numeric_limits<size_t>::max();
                }
            } else {
                // Distance between chains
                edge_dist = SnarlDistanceIndex::sum(between_chain_dist, chain_flank_dist);
            }
            dist_matrix.emplace_back(ZipCodeTree::EDGE, edge_dist, false);
        }

        // start ->
        between_chain_dist = first_seed.zipcode.get_distance_to_snarl_bound(depth+1, !snarl_is_reversed, seeds[i].side);
        dist_matrix.emplace_back(ZipCodeTree::EDGE, SnarlDistanceIndex::sum(between_chain_dist, seeds[i].flank_offset), false);
    }

    // end -> end
    dist_matrix.emplace_back(ZipCodeTree::EDGE, 
        forest_state.distance_index->distance_in_snarl(snarl_handle, 1, false, 1, false), false);

    // Finally, distances involving the snarl end
    for (size_t i = 0; i < seeds.size(); ++i) {
        const auto& cur_seed = forest_state.seeds->at(seeds[i].index);
        size_t to_chain_edge = cur_seed.zipcode.get_distance_to_snarl_bound(depth+1, snarl_is_reversed, seeds[i].side);
        dist_matrix.emplace_back(ZipCodeTree::EDGE, SnarlDistanceIndex::sum(to_chain_edge, seeds[i].flank_offset), false);
    }
    ...
    */
    ...
    for (size_t i = 0 ; i < zip_code_tree.size() ; i++ ) {
        ...
        if (current_item.is_snarl_start()) {
            ...
            current_depth++;
        } else if (current_item.is_chain_end() || current_item.is_snarl_end()) {
    ...
    for (size_t i = 0 ; i < zip_code_tree.size() ; i++) {
        ...
        if (item.is_snarl_start()) {
            if (snarl_stack.size() == 1) {
                ...
                std::vector<vg::ZipCodeTree::tree_item_t>::const_iterator cur_snarl_start = zip_code_tree.begin() + i;
                validate_snarl(cur_snarl_start, distance_index, seeds, distance_limit);
            }
            if (item.get_type() == CYCLIC_SNARL_START) {
                if (!cyclic_snarl_id_stack.empty()) {
                    // Cyclic snarl IDs must be in increasing order
        ...
        } else if (item.get_type() == CHAIN_START || item.get_type() == CYCLIC_SNARL_CHAIN_START) {
            if (item.get_type() == CYCLIC_SNARL_CHAIN_START) {
                assert(cyclic_snarl_id_stack.back() == item.get_value());
            }
            snarl_stack.push_back(item.get_type());
        } else if (item.get_type() == SNARL_END) {
...
void ZipCodeTree::validate_snarl(std::vector<tree_item_t>::const_iterator& zip_iterator, 
    ...
    if (zip_iterator->get_type() == SNARL_START) {
        ...
        assert(zip_iterator->get_type() == CYCLIC_SNARL_END);
    }
}
...
void ZipCodeTree::validate_non_cyclic_snarl(std::vector<tree_item_t>::const_iterator& zip_iterator, 
                                            ...
                                            size_t distance_limit) const {
#ifdef DEBUG_ZIP_CODE_TREE
    std::cerr << "Validating non cyclic snarl" << std::endl;
#endif
    assert(zip_iterator->get_type() == SNARL_START);
    ...
    ranks.emplace_back(0, false);
    zip_iterator++;
    while (zip_iterator->get_type() != NODE_COUNT) {
        if (zip_iterator->get_type() == EDGE) {
            dist_matrix.emplace_back(zip_iterator->get_value());
            ...
            zip_iterator--;
            store_seed_position_and_rank(*zip_iterator, distance_index, seeds, positions, ranks);
            // Skip forward to the end of the chain again
            zip_iterator++;
        }
        zip_iterator++;
    }
    ...
    assert(positions.size()-2 == zip_iterator->get_value());
    zip_iterator++;
    assert(zip_iterator->get_type() == SNARL_END);

    validate_distance_matrix(distance_index, dist_matrix, positions, ranks, false, distance_limit);
};
...
void ZipCodeTree::validate_cyclic_snarl(std::vector<tree_item_t>::const_iterator& zip_iterator, 
...
#endif
    // Snarl header
    assert(zip_iterator->get_type() == CYCLIC_SNARL_START);
    ...
    vector<size_t> dist_matrix;
    while (zip_iterator->get_type() == EDGE) {
    ...
    while (zip_iterator->get_type() != CYCLIC_SNARL_END) {
        assert(zip_iterator->get_type() == CYCLIC_SNARL_CHAIN_START);
        assert(zip_iterator->get_value() == snarl_id);
        // Skip forward to first child
        zip_iterator++;
        store_seed_position_and_rank(*zip_iterator, distance_index, seeds, positions, ranks, true);
        ...
        // Back up to last child
        zip_iterator--;
        store_seed_position_and_rank(*zip_iterator, distance_index, seeds, positions, ranks, false);
    ...
    validate_distance_matrix(distance_index, dist_matrix, positions, ranks, true, distance_limit);
}

void ZipCodeTree::store_seed_position_and_rank(tree_item_t child, 
                                               const SnarlDistanceIndex& distance_index, 
                                               const vector<Seed>* seeds,
    ...
    if (child.get_type() == SEED) {
        ...
        ranks.emplace_back(distance_index.get_rank_in_parent(seed_handle), is_reversed);
    } else {
...
void ZipCodeTree::validate_distance_matrix(const SnarlDistanceIndex& distance_index,
    ...
    for (size_t i = 0; i < positions.size(); i++) {
        pos_t from_pos = positions[i];
        if (id(from_pos) == 0) {
            // Skip the placeholder
            continue;
        }

        size_t end_index = has_self_loops ? i : i - 1;
        for (size_t j = 0; j <= end_index; j++) {
            pos_t to_pos = positions[j];
            if (id(to_pos) == 0) {
                // Skip the placeholder
                continue;
            }
            net_handle_t to_handle = distance_index.get_node_net_handle(id(to_pos), is_rev(to_pos));
            cerr << "to_pos: " << id(to_pos) << " " << is_rev(to_pos) << endl;
            net_handle_t from_handle = distance_index.get_node_net_handle(id(from_pos), is_rev(from_pos));
            cerr << "from_pos: " << id(from_pos) << " " << is_rev(from_pos) << endl;
            net_handle_t parent_handle;
            size_t true_distance;
            if (distance_index.get_parent(to_handle) == distance_index.get_parent(from_handle)) {
                cerr << "Same parent" << endl;
                ...
                cerr << "Different parents" << endl;
                // Go another level up
                //to_handle = distance_index.get_parent(to_handle);
                //from_handle = distance_index.get_parent(from_handle);
                parent_handle = distance_index.get_parent(distance_index.get_parent(to_handle));
                true_distance = distance_index.minimum_distance(id(to_pos), is_rev(to_pos), offset(to_pos),
                    id(from_pos), is_rev(from_pos), offset(from_pos), true);
                if (true_distance != std::numeric_limits<size_t>::max() && is_rev(from_pos)) {
                    //If the seed we ended at got reversed, then add 1
                    true_distance += 1;
                }
                if (true_distance != std::numeric_limits<size_t>::max() && is_rev(to_pos)) {
                    //If the seed we're starting from got reversed, then subtract 1
                    true_distance -= 1;
                }
            ...
            true_distance = distance_index.distance_in_parent(parent_handle, to_handle, from_handle);
            cerr << "true_distance: " << true_distance << endl;
            // Add distance within node for to_handle
            true_distance = SnarlDistanceIndex::sum(true_distance, distance_index.minimum_length(to_handle) - offset(to_pos));
            cerr << "added: " << distance_index.minimum_length(to_handle) - offset(to_pos) << endl;
            // Add distance within node for from_handle
            true_distance = SnarlDistanceIndex::sum(true_distance, distance_index.minimum_length(from_handle) - offset(from_pos));
            cerr << "added: " << distance_index.minimum_length(from_handle) - offset(from_pos) << endl;
...
void ZipCodeTree::validate_chain(std::vector<tree_item_t>::const_iterator& zip_iterator, 
                                  const SnarlDistanceIndex& distance_index, 
                                  const vector<Seed>* seeds,
                                  size_t distance_limit) const {
#ifdef DEBUG_ZIP_CODE_TREE
    ...
    while (!zip_iterator->is_chain_end()) {
...
auto ZipCodeTree::reverse_iterator::tick() -> bool {
    ...
    case S_STACK_SNARL:
        ...
        case EDGE:
            ...
            swap();
            break;
        case CYCLIC_SNARL_CHAIN_END: // For now we treat these like regular snarls
        case CHAIN_END:

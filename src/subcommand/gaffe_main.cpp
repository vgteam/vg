...
int main_gaffe(int argc, char** argv) {
    ...
    while (true) {
        ...
        int option_index = 0;
        c = getopt_long (argc, argv, "hx:m:s:d:c:G:f:M:t:",
    ...
    unique_ptr<DistanceIndex> distance_index;
    {
        ...
        if (!distance_stream) {
            cerr << "error:[vg gaffe] Could not open " << distance_name << endl;
            exit(1);
        }
    ...
    auto map_read = [&](Alignment& aln) {
        ...
        for (size_t i = 0; i < min(cluster_indexes_in_order.size(), max_multimaps); i++) {
            // For each cluster, produce an Alignment
            aligned.emplace_back(aln);
            Alignment& out = aligned.back();
            
            // TODO: Call Jouni's extender here
            out.set_score(0);
            out.clear_path();
            out.clear_refpos();

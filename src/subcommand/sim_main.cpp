...
vector<pair<string, double>> parse_rsem_expression_file(istream& rsem_in) {
    ...
    while (getline(rsem_in, line)) {
        ...
        if (tokens.size() != 8) {
            fatal_error(context) << "Cannot parse transcription file. "
...
vector<tuple<string, string, size_t>> parse_haplotype_transcript_file(istream& haplo_tx_in) {
    ...
    while (getline(haplo_tx_in, line)) {
        ...
        if (tokens.size() != 5) {
            fatal_error(context) << "Cannot parse haplotype transcript file. "
...
int main_sim(int argc, char** argv) {
    ...
    while (true) {
        ...
        case 'R':
            for (auto& rule : split_delims(optarg, ",")) {
                ...
                if (parts.size() != 2) {
                    fatal_error(context) << "ploidy rules must be REGEX:PLOIDY" << std::endl;
                }
                try {
                    // Parse the regex
        ...
        case 's':
            ...
            if (seed_val == 0) {
                ...
                fatal_error(context) << "seed 0 cannot be used. Omit the seed option "
    ...
    if (align_out && fastq_out) {
        fatal_error(context) << "only one output format (-a/-J or -q) can be selected." << std::endl;
    }

    if (xg_name.empty()) {
        fatal_error(context) << "we need a graph to sample reads from" << std::endl;
    }
    if (!gbwt_name.empty() && sample_names.empty() && rsem_file_name.empty()) {
        fatal_error(context) << "--gbwt-name requires --sample-name or --tx-expr-file" << std::endl;
    }
    if (!gbwt_name.empty() && !rsem_file_name.empty() && !haplotype_transcript_file_name.empty()) {
        // TODO: This message doesn't really make sense.
        fatal_error(context) << "using --gbwt-name requires that HSTs be included --tx-expr-file; "
    ...
    if (!path_pos_filename.empty() && fastq_name.empty()) {
        fatal_error(context) << "path usage table is not available unless using trained simulation (-F)" << std::endl;
    }
    
    if (fastq_name.empty() && unsheared_fragments) {
        fatal_error(context) << "unsheared fragment option only available "
    ...
    if (any_path) {
        ...
        if (path_handle_graph->get_path_count() == 0) {
            fatal_error(context) << "the graph does not contain paths" << std::endl;
        }
        path_names.clear();
        ...
        for (auto& path_name : path_names) {
            if (path_handle_graph->has_path(path_name) == false) {
                fatal_error(context) << "path \"" << path_name << "\" not found in index" << std::endl;
            }
    ...
    if (!gbwt_name.empty()) {
        ...
        if (!(gbwt_index->hasMetadata()) || !(gbwt_index->metadata.hasSampleNames()) 
            ...
            fatal_error(context) << "GBWT index does not contain sufficient metadata" << std::endl;
        }
        ...
        if (!sample_names.empty()) {
            ...
            for (std::string& sample_name : sample_names) {
                ...
                if (id >= gbwt_index->metadata.samples()) {
                    fatal_error(context) << "sample \"" << sample_name 
            ...
            for (const auto& transcript_expression  : transcript_expressions) {
                ...
                if (id >= gbwt_index->metadata.samples()) {
                    fatal_error(context) << "haplotype-specific transcript \"" << transcript_expression.first
                                         << "\" not found in the GBWT index" << std::endl;
                }
                auto idx = sample_id_to_idx.size();
                sample_id_to_idx[id] = idx;
        ...
        if (mutable_graph == nullptr) {
            if (progress) {
                basic_log(context)  << ": Converting the graph into HashGraph" << std::endl;
            }
        ...
        if (!sample_names.empty()) {
            ...
            if (sample_name_set.size() != sample_names.size()) {
                ...
                fatal_error(context) << "Of the " << sample_names.size()
    ...
    if (haplotype_transcript_file_name.empty()) {
        if (!transcript_expressions.empty()) {
            ...
            for (auto& transcript_expression : transcript_expressions) {
                if (!path_handle_graph->has_path(transcript_expression.first)) {
                    fatal_error(context) << "transcript path \"" << transcript_expression.first 
        ...
        for (auto& haplotype_transcript : haplotype_transcripts) {
            if (!path_handle_graph->has_path(get<0>(haplotype_transcript))) {
                fatal_error(context) << "transcript path for \"" << get<0>(haplotype_transcript) 
    ...
    if (fastq_name.empty()) {
        ...
        if (unsheared_fragments) {
            warning(context) << "Unsheared fragments option is only available "
                             << "when simulating from FASTQ-trained errors" << std::endl;
        }

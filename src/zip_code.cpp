...
tuple<ZipCode::code_type_t, size_t, size_t, size_t, bool> ZipCode::get_chain_child_sort_info(const size_t& depth) const {
#ifdef DEBUG_ZIPCODE
    // Check our assumptions
    assert(NODE_OFFSET_OFFSET == 0);
    assert(NODE_LENGTH_OFFSET == 1);
    assert(NODE_IS_REVERSED_OFFSET == 2);
    assert(NODE_CHAIN_COMPONENT_OFFSET == 3);

    assert(SNARL_IS_REGULAR_OFFSET == 0);
    assert(SNARL_OFFSET_IN_CHAIN_OFFSET == 1);
    assert(SNARL_LENGTH_OFFSET == 2);
    assert(SNARL_CHAIN_COMPONENT_OFFSET == 4);
#endif
    if (depth == 0) {
    ...
    if (decoder[depth].is_chain) {
        ...
        if (decoder[depth-1].is_chain) {
            // If the thing before this was also a chain, then it is a node
            code_type = ZipCode::NODE;
            size_t zip_value;
            size_t zip_index = decoder[depth].offset;
            // First is the offset
            std::tie(zip_value, zip_index) = zipcode.get_value_and_next_index(zip_index);
            offset_in_chain = zip_value == 0 ? std::numeric_limits<size_t>::max() : zip_value-1;
            // Next is the length
            std::tie(zip_value, zip_index) = zipcode.get_value_and_next_index(zip_index);
            length = 0 ? std::numeric_limits<size_t>::max() : zip_value-1;
            // Next is the reversedness
            std::tie(zip_value, zip_index) = zipcode.get_value_and_next_index(zip_index);
            is_reversed = zip_value;
        ...
        if (zip_value == 0) {
            ...
            code_type = ZipCode::REGULAR_SNARL;
        } else {
        ...
        std::tie(zip_value, zip_index) = zipcode.get_value_and_next_index(zip_index);
        length = 0 ? std::numeric_limits<size_t>::max() : zip_value-1;
        // Two from now is the chain component
        std::tie(zip_value, zip_index) = zipcode.get_value_and_next_index(zip_index);
        std::tie(zip_value, zip_index) = zipcode.get_value_and_next_index(zip_index);
        chain_component = zip_value;
        // Snarls are always considered to be forward
        is_reversed = false;
    }

    return std::make_tuple(code_type, offset_in_chain, length, chain_component, is_reversed);
}

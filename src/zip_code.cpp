    ...
    for (int i = ancestors.size()-1 ; i >= 0 ; i--) {
        ...
        if (distance_index.is_node(current_ancestor)) {
            ...
            zipcode.add_value(node_code.get_raw_is_reversed()
                              + (node_code.get_raw_has_forward_loop() << 1)
                              + (node_code.get_raw_has_reverse_loop() << 2));
...
cerr << "\tThe last thing was a root-level node, so nothing else" << endl;
        ...
        if (previous_is_chain) {
            ...
            // Access ones bit, which is regular/irregular
...
bool ZipCode::get_is_reversed_in_parent(const size_t& depth) const {
    ...
    if (depth == 0) {
        ...
        if (decoder[depth-1].is_chain) {
            ...
            size_t zip_value;
            size_t zip_index = decoder[depth].offset;
            for (size_t i = 0 ; i <= ZipCode::NODE_FLAGS_OFFSET ; i++) {
                std::tie(zip_value, zip_index) = zipcode.get_value_and_next_index(zip_index);
            }
            return zip_value & ZipCode::NODE_FLAG_IS_REVERSED;
        } else {
...
bool ZipCode::get_has_forward_loop(const size_t& depth) const {
    if (depth == 0) {
        ...
        return zip_value & ZipCode::NODE_FLAG_CAN_FORWARD_LOOP;
    } else if (!decoder[depth].is_chain) {
        //If this is a snarl
        size_t zip_value;
        size_t zip_index = decoder[depth].offset;
        ...
        return zip_value & ZipCode::SNARL_FLAG_CAN_FORWARD_LOOP;
    } else {
        // If this is a chain
        return false;
    }
...
bool ZipCode::get_has_reverse_loop(const size_t& depth) const {
    if (depth == 0) {
        ...
        return zip_value & ZipCode::NODE_FLAG_CAN_REVERSE_LOOP;
    } else if (!decoder[depth].is_chain) {
        //If this is a snarl
        size_t zip_value;
        size_t zip_index = decoder[depth].offset;
        ...
        return zip_value & ZipCode::SNARL_FLAG_CAN_REVERSE_LOOP;
    } else {
        // If this is a chain
        return false;
    }
...
bool ZipCode::is_externally_start_end_connected (const size_t& depth) const {
    ...
    return zip_value & ZipCode::START_END_CONNECTIVITY_FLAG;
}
bool ZipCode::is_externally_start_start_connected (const size_t& depth) const {
    ...
    return zip_value & ZipCode::START_START_CONNECTIVITY_FLAG;
}
bool ZipCode::is_externally_end_end_connected (const size_t& depth) const {
    ...
    return zip_value & ZipCode::END_END_CONNECTIVITY_FLAG;
}
...
ZipCode::snarl_code_t ZipCode::get_regular_snarl_code(const net_handle_t& snarl, const net_handle_t& snarl_child, const SnarlDistanceIndex& distance_index) {
    ...
    net_handle_t snarl_start = distance_index.get_node_from_sentinel(distance_index.get_bound(snarl, false, false));
...
ZipCode::node_code_t ZipCode::unpack_node_code(size_t zipcode_level) const {
    ...
    if (zipcode_level == 0) {
        ...
        //Is reversed
        std::tie(zip_value, zip_index) = zipcode.get_value_and_next_index(zip_index);
        node_code.set_raw_is_reversed(zip_value & ZipCode::NODE_FLAG_IS_REVERSED);
        node_code.set_raw_has_forward_loop(zip_value & ZipCode::NODE_FLAG_CAN_FORWARD_LOOP);
        node_code.set_raw_has_reverse_loop(zip_value & ZipCode::NODE_FLAG_CAN_REVERSE_LOOP);
        //Chain component
        std::tie(zip_value, zip_index) = zipcode.get_value_and_next_index(zip_index);
...
ZipCode::snarl_code_t ZipCode::unpack_snarl_code(size_t zipcode_level) const {
    ...
    if (zipcode_level == 0) {
        ...
        //Snarl is regular
        std::tie(zip_value, zip_index) = zipcode.get_value_and_next_index(zip_index);
        // First two bits indicate the snarl type
        snarl_code.set_raw_code_type(zip_value & 3);
        // Next two bits indicate loopability
        snarl_code.set_raw_has_forward_loop(zip_value & ZipCode::SNARL_FLAG_CAN_FORWARD_LOOP);
        snarl_code.set_raw_has_reverse_loop(zip_value & ZipCode::SNARL_FLAG_CAN_REVERSE_LOOP);
...
void ZipCode::fill_in_zipcode_from_payload(const code_type* payload) {
    ...
    bool is_trivial_chain = is_chain_val & 2;
...
MIPayload ZipCode::get_payload_from_zipcode(nid_t id, const SnarlDistanceIndex& distance_index, hash_map<size_t, net_handle_t>* component_to_net_handle) const {
    ...
    if (decoder_length() == 1) {
        ...
        payload.is_reversed = zip_value & ZipCode::NODE_FLAG_IS_REVERSED;

...
std::int64_t Homogenizer::sv_concordance(vg::Alignment& aln){
	std::int64_t concordance = 0;
	vg::Filter ff;
	// Get soft-clipped bases
	std::string clipseq = ff.get_clipped_seq(aln);
	std::size_t cliplen = clipseq.length();
	concordance = -1 * cliplen;
	return concordance;
}
std::int64_t Homogenizer::sv_concordance(pair<Alignment&, Alignment&> alns){
	int64_t concordance= 0;

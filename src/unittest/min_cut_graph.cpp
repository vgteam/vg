...
namespace vg {
    namespace unittest {
        ...
        TEST_CASE("Min_cut2") {
            ...
            SECTION("Test2: Can find a min-cut on a 4 node graph") {
                /* Let us create following undirected, weighted graph  
                      e1 10  
                    1--------2  
                    | \      |  
                ...
                */
                ...
                //other node
                //uses subscripts starting from 0
                edge12.other = 1;
                edge21.other = 0;
                edge13.other = 2;
                edge31.other = 0;
                ...
                //Karger's min-cut will minimize the total weight of the cut 
                pair<vector<vector<size_t>>, size_t> to_recv= compute_min_cut(graph, n_iterations, seed, V);
                vector<vector<size_t>> disjoint_sets = to_recv.first;
                //returns the number of minimum edge cuts required to devide graph into two disjoint connected subgraphs 
                // REQUIRE(to_recv.second == 2);

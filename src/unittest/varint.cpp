...
using namespace std;
    ...
    TEST_CASE("Array of ints", "[varint]") {
        SECTION ("[0]") {
            ...
            pair<int64_t, size_t> value_and_index = varint_vector.get_value_and_next_index(0);
        ...
        SECTION ("more values") {
            cerr << endl;
            vector<int64_t> values {1, 56435345, 23423, 5, 123498275, 0, 213, 14253452324, std::numeric_limits<int64_t>::max(), 0, 23123241234234, std::numeric_limits<int64_t>::max()-1};
            varint_vector_t varint_vector;
            for (auto& x : values) {
            ...
            while (i < values.size()) {
                pair<int64_t, size_t> value_and_index = varint_vector.get_value_and_next_index(index);

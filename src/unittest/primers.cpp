    ...
    namespace unittest {
        ...
        TEST_CASE( "filter simple primers",
            ...
            public:
                ...
                // starting position relative to the starting node
                Primer_pair add_primer_pair(size_t left_primer_starting_node_id,
                ...
                void run_test() {
                        ...
                        cout << "Testing PathPositionHandleGraph..." << endl;
                        string left_primer_sequence;
                        handle_t starting_handle = graph->get_handle(3);
                        size_t left_primer_offset = 1;
                        size_t left_primer_length = 10;
                        step_handle_t starting_step_handle = graph->steps_of_handle(starting_handle)[0];
                        left_primer_sequence += graph->get_sequence(starting_handle).substr(left_primer_offset,
                            graph->get_length(starting_handle) - left_primer_offset);
                        cout << left_primer_sequence << endl;
                        ...
                        add_primer_pair(3, 1, 10, 8, 3, 15);
                        add_primer_pair(9, 14, 20, 22, 0, 20);
                        for (vector<Primer_pair>::iterator it = primer_pairs.begin(); it != primer_pairs.end(); ++it) {
                            ...
                            cout << "offsets: " << left_primer.offset << " " << right_primer.offset << endl;
                            cout << "product size: " << it->linear_product_size << endl;
                            
                            size_t min_product_size = distance_index->minimum_distance(left_primer.mapped_nodes_ids[0],
                                false, left_primer.offset, right_primer.mapped_nodes_ids[right_primer.mapped_nodes_ids.size()-1], 
                                false, right_primer.offset);
                            cout << "minimum product size: " << min_product_size << endl;
                            
                            size_t max_product_size = distance_index->maximum_distance(left_primer.mapped_nodes_ids[0],
                                false, left_primer.offset, right_primer.mapped_nodes_ids[right_primer.mapped_nodes_ids.size()-1], 
                                false, right_primer.offset);
                            cout << "maximum product size: " << max_product_size << endl;
                        ...
                        for (vector<Primer_pair>::iterator it = selected_primer_pairs.begin(); it != selected_primer_pairs.end(); ++it) {
                            Primer left_primer = it->left_primer;
                            Primer right_primer = it->right_primer;
                            cout << left_primer.sequence << " - " << right_primer.sequence << endl;
                        }
            ...
            private:
                ...
                size_t longest_match_len(Primer& primer, string const left_seq, string const right_seq, bool const first_node) {

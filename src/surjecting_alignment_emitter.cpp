...
SurjectingAlignmentEmitter::SurjectingAlignmentEmitter(const PathPositionHandleGraph* graph, unordered_set<path_handle_t> paths,
    ...
    bool add_off_ref_position_tag, bool left_align, bool rescue_secondary) : surjector(graph), paths(paths), backing(std::move(backing)) {
...
void SurjectingAlignmentEmitter::emit_mapped_singles(vector<vector<Alignment>>&& alns_batch) {
    ...
    for (auto& mappings : alns_batch_caught) {
        ...
        // rescue a secondary if the primary failed to surject.
        surjector.rescue_secondary_if_primary_unmapped(mappings);
    }
    // Forward it along
...
void SurjectingAlignmentEmitter::emit_mapped_pairs(vector<vector<Alignment>>&& alns1_batch, vector<vector<Alignment>>&& alns2_batch, vector<int64_t>&& tlen_limit_batch) {
    ...
    for (size_t i = 0; i < alns1_batch_caught.size(); ++i) {
        ...
        // pair). If both mates of the primary pair failed to surject, rescue
        // the best surjectable secondary pair by tagging it with YF:i:1.
        surjector.rescue_secondary_pair_if_primary_unmapped(alns1_batch_caught[i], alns2_batch_caught[i]);
    }
    // Forward it along

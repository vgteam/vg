...
struct GaplessMatch {
    ...
    bool operator<(const GaplessMatch& another) const {
        return ((this->mismatches > another.mismatches) ||
                (this->mismatches == another.mismatches && this->length() < another.length()));
    }
};

std::ostream& operator<<(std::ostream& out, const GaplessMatch& match) {
    out << "(" << match.mismatches << ", (" << match.start << ", " << match.limit << "), ";
...
void backward_mismatches(const std::string& seq, std::pair<const char*, size_t> target,
    ...
    while (match.start > 0 && target_offset > 0 && match.mismatches < error_bound) {
        match.start--;
        target_offset--;
        if (seq[match.start] != target.first[target_offset]) {
            match.mismatches++;
        }
    }
}
...
bool gapless_extension(const GBWTGraph& graph, const std::string& seq, const std::vector<std::pair<size_t, pos_t>>& hits, size_t max_errors) {
    ...
    // Consider each hit as a seed.
    size_t best_match = max_errors + 1;
    pos_t prev = make_pos_t(0, false, 0);
    for (size_t i = 0; i < hits.size(); i++) {
        // Start matching as early in the initial node as possible.
        std::pair<size_t, pos_t> hit = hits[i];
        ...
        GaplessMatch match {
            ...
            hit.first, hit.first,
            graph.get_bd_state(handle)
        };
        forward_mismatches(seq, graph.get_sequence_view(handle), offset(hit.second), match, best_match);
        if (match.mismatches >= best_match) { 
            continue;
        }
        std::priority_queue<GaplessMatch> forward, backward;
        if (match.limit >= seq.length()) {
            if (match.start == 0) {
                best_match = std::min(best_match, match.mismatches);
            } else {
                backward.push(match);
            }
        } else {
        ...
        while (!forward.empty()) {
            ...
            graph.follow_paths(curr.state, false, [&](const gbwt::BidirectionalState& next_state) -> bool {
                ...
                handle_t h = GBWTGraph::node_to_handle(next_state.forward.node);
                forward_mismatches(seq, graph.get_sequence_view(h), 0, next, best_match);
                if (next.mismatches >= best_match) {
                    return true;
                }
                if (next.limit >= seq.length()) {
                    if (next.start == 0) {
                        best_match = std::min(best_match, next.mismatches);
                    } else {
                        backward.push(next);
                    }
                } else {
        ...
        while (!backward.empty()) {
            ...
            graph.follow_paths(curr.state, true, [&](const gbwt::BidirectionalState& next_state) -> bool {
                ...
                handle_t h = GBWTGraph::node_to_handle(gbwt::Node::reverse(next_state.backward.node));
                backward_mismatches(seq, graph.get_sequence_view(h), next, best_match);
                if (next.mismatches >= best_match) {
                    return true;
                }
                if (next.start == 0) {

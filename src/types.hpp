...
typedef std::tuple<id_t, bool, vg::off_t> pos_t;

/// Create a pos_t from a Node ID, an orientation flag, and an offset along that strand of the node.
inline pos_t make_pos_t(id_t id, bool is_rev, vg::off_t off) {
...
inline vg::off_t offset(const pos_t& pos) {
...
inline vg::off_t& get_offset(pos_t& pos) {
    return std::get<2>(pos);
}

...
public:
    ...
    /// Get the deduplicated scopes, in addition order, that the given index
    /// will be qualified with when generated. Only works for indexes that will
    /// be generated by the plan, not for inputs.
    const vector<string> get_scopes(const IndexName& identifier) const;
    ...
    ///
    /// Requires that all indexes in the plan be generated by a recipe; inputs
    /// must not be in the plan as steps in order to call this.
...
protected:
    ...
    void add_scope(const IndexName& identifier, const string& scope);
...
public:
    ...
    /// Append a new scope to qualify the files in the index, if the scope is
    ...
    /// IndexingPlan.
    void add_scope(const string& scope);
    
    /// Get all scopes qualifying the index (such as a sample name).
    /// Scopes are deduplicated and produced in the order added.
    const vector<string>& get_scopes() const;
...
private:
    ...
    // The dedplicated ordered scopes qualifying the index.
    // TODO: Use an ordered set for good algorithmics for deduplicating scopes.
    // Right now we have exactly one kind of scope so this doesn't matter.

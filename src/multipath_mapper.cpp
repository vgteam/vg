...
//            }
    ...
    void MultipathMapper::multipath_align(const Alignment& alignment, VG* vg,
        ...
        if (synthetic_anchor_search_limit > -1) {
            ...
            // original anchors. This has to happen *after* snarl cutting
            // because snarl cutting won't cut anchors fully within snarls.
        ...
        for (size_t i = 0; i < multipath_alns.size(); i++) {
            ...
            if (query_population) {
                ...
                if (!top_tracebacks && haplo_score_provider->has_incremental_search()) {
                    ...
                    alignments = haplotype_consistent_alignments(multipath_alns[i], *haplo_score_provider,
                        population_max_paths, true, population_hard_cap);
                } else {
        ...
        for (size_t i = 0; i < multipath_aln_pairs.size(); i++) {
            ...
            if (query_population) {
                ...
                if (!top_tracebacks && haplo_score_provider->has_incremental_search()) {
                    ...
                    alignments[0] = haplotype_consistent_alignments(multipath_aln_pair.first, *haplo_score_provider,
                        population_max_paths, true, population_hard_cap);
                    alignments[1] = haplotype_consistent_alignments(multipath_aln_pair.second, *haplo_score_provider,
                        population_max_paths, true, population_hard_cap);
                } else {
                    // We will just find the top n best-alignment-scoring linearizations and hope some match haplotypes
